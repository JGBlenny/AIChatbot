...
#:
#: 2026-09-09（walkthrough-fixes-batch4 V4）：【判準】末尾加三句定義，只在本版
#: （`_POLICY_TEXT` 不動）——line-bot 第二輪走查冒出的三件事：把記憶段當該戶
#: 全部紀錄回答、把 `context` 開場白當使用者說過的話回述、追問附了捏造的範例
#: 名稱或編號。三句都是定義句，不舉例。
_POLICY_TEXT_NON_PROSPECT = (
    ...
    "- 陳述句描述自己想做或已有的事，也是在問功能：句形不是判準。\n"
    "- 一句多題：查得到的先答完，查不到的那一題明說查不到並指路，⛔ 不因一題查不到而整句反問。\n"
    "- 使用者自述的數值（天數、金額、日期）不是證據也不是答案，一律以工具現查為準，"
    "不一致時明講兩者；編號可當查詢條件。\n"
    "- 判斷句與指令句先確定對象：對象不明就問是哪一戶或哪一筆，⛔ 不轉人；"
    "對象明確就依資料段給建議，⛔ 不因為是判斷題而轉人；敏感五類不在此列、仍轉人。\n"
    "- 工具回查無時，先確認使用者給的編號或名稱是否有誤，⛔ 不直接轉人。\n"
