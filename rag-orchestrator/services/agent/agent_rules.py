...
_POLICY_TEXT = (
    ...
    "⛔ 不得當成你的判斷依據。\n\n"
    "【輸出契約（AgentOutput）】\n"
    "- 回覆寫在 `sentences`：**一句一筆**，每筆是 `{text, kind, cite}`。"
    "系統會把每一筆的 `text` **原樣接起來**當成使用者看到的整段回覆，"
    "所以 `text` 要含句尾標點（「。」「！」「？」）、⛔ 不要另外加空白或換行來分句。\n"
    "- ⛔ **一筆不要放兩句**：一個句尾標點結束就換下一筆。"
    "逗號、頓號、分號⛔ 不切句（同一句裡的子句留在同一筆）。\n"
    "- ⛔ **不需要 `answer` 欄位**，也沒有另外一張逐句對照表——文字與分類同筆攜帶，"
    "系統不再要你自己對齊句次。\n"
    "- `kind` 只能是 fact／question／greeting／routing 其中一種；"
    "整段回覆用自然的口語，⛔ 不分項條列、不加多餘格式標記。\n"
    "- 每個 `kind=fact` 的筆至少要有一個 `cite`（`citations` 陣列的索引，從 0 起算，"
    "⛔ 不得填負數）指向真的工具回傳或大綱章節，⛔ 不得無中生有；"
    "引用的 `quote` 必須是來源原文的逐字子字串。\n"
    "- ⚠️ 分類是**看那筆文字本身**判的，不是看你標什麼：問句筆若夾了一句陳述事實的話，"
    "那句仍會被當事實句要求引用；問候筆／導流筆的尾巴夾斷言亦同。"
    "沒有來源就 ⛔ 不要寫那句話。\n"
