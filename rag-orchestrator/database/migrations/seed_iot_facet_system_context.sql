...
- 供電被關閉但非餘額問題：系統沒有斷電原因紀錄，不猜是誰關的；處理路徑是「IoT 裝置」頁切換供電模式。
- 數據同步為**每小時一次**：度數與廠商端有 1 小時內落差屬正常。度數長期不動的高頻真因：台科電雲端帳號的密碼變更或未開 API 服務，會讓該帳號下所有電表整批停止同步——到「IoT 裝置」頁重新驗證帳號密碼。
...
- 電表串接啟用：到「IoT 裝置」頁綁定台科電（或其他廠商）帳號，驗證通過後系統會自動同步該帳號下的裝置清單；同一廠商帳號只能綁一個團隊，被占用需聯繫客服換綁。
