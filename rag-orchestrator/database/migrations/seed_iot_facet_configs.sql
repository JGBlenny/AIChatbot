...
【不代操作紅線】復電、斷電、模式切換、儲值都不代執行——只說明操作路徑（裝置管理→智慧電錶）或引導租客儲值；系統判定說「自動復電」就照講，不承諾時間以外的結果。
...
每輪輸出 JSON：{"action":"ask"|"converge","converge_kind":"answer","extracted_fields":{"meter_ref":"…","symptom":"…（如有）"},"next_question":"…","scope":"stay"|"switch","face":"…（如有）"}$RULES$,
    ...
    '{
        "conversational_config": {
            ...
            "answer_rules": "## 收斂作答鐵則\n- 底稿開頭即對方問的那顆電表，照底稿系統判定直接作答，不推託、不自加原因。\n- 離線態照底稿快照措辭講（截至最後同步），不下供電結論。\n- 不代操作：復電/斷電/模式切換/儲值只指路（裝置管理→智慧電錶，或請租客儲值），不代執行、不承諾代辦。\n- 度數/餘額/單價只引用底稿存值，禁止計算或改寫數字。\n- 電費計收紅線：儲值電表為租客預付制（租客自行儲值扣抵用電），業者不需另向租客收電費——絕不建議「照餘額/度數計費向租客收取」；帳單類收費問題屬帳務範疇。\n- 度數正確性照脈絡講：度數為台科電回傳存值、系統不重算，疑義請台科電調閱設備紀錄。\n- 追問輪只答新問題，不整段重複已述的電表現值。\n- 底稿導廠商（台科電等）時照導，附電表名稱與物件資訊。\n- 一切以 API 現值為準，脈絡通則不覆寫這顆電表的資料。",
            "grounding_scope": {
                "select": "api",
                "endpoint": "jgb_meters",
...
【分流確認】設定對象不明時先用一句分流（電表串接？儲值/單價？電費計收？門鎖密碼規則？連結起始日？放 extracted_fields.topic），至多兩輪；已講明不重問。問句本身已含明確主題（如「電費要不要跟租客收」「單價可以改嗎」）就是情境清楚，直接收斂不分流。
【收斂】情境清楚即 action="converge"、converge_kind="answer"，依系統脈絡的設定框架給對的路徑（綁定廠商帳號、單價在台科電端設定、儲值流程與帳單效期、電費計收採租客預付制）。
