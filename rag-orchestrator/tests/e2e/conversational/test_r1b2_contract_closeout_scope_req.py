"""e2e：B″ runtime-delegated candidate expansion（spec face-exit-before-grounding）。

B′ 的 authoritative in-session evaluation **明確**把本 query delegate 至
`contract_closeout`（「點退帳單屬合約退租收尾範疇…轉由合約領域接手」）。
依 R9 的 candidate-set closure 規則納入該 Face 為候選——**不是**研究者手選。

> 這一刀要分開的是兩個完全不同的治理問題：
> 「責任配置沒有 owner」 vs 「owner 存在，但 routing 永遠送不到它」。

凍結協議：`.kiro/specs/face-exit-before-grounding/r1b2-protocol-frozen.md`
（受測 Face 的 rules／in-session ctx digest、model／config、3 次、retry、
 target≤8・all_provider≤20、**三格裁決表**與 3 stay／3 switch 的續分流，皆先於執行凍結。）

⚠️ B／B′ 的紀錄不動、不回填。全程 production path（direct entry 自建 session），
不人工餵 context，`PREENTRY_ROUTABILITY_GATE` 維持 false。
...
QUERY = "幫我查點退帳單金額"
FACE = "contract_closeout"
PERSONA = "pm_contract_closeout"
EXPECTED_INSESSION_CTX = "16b107f0aaf4eb5f"     # 協議 §3 的成立條件（退租收尾）
EXPECTED_RULES_DIGEST = "363ed0d0da3ca23c"      # 以此區分 contract_closeout 的那次呼叫
...
EVIDENCE_PATH = os.path.join(os.path.dirname(__file__), "..", "..", ".r1b2_evidence.json")
...
_EVIDENCE = {"protocol": "r1b2-protocol-frozen.md", "variant": "B-double-prime",
             "query": QUERY, "query_sha": _sha(QUERY), "repetitions": REPETITIONS, "runs": []}


@pytest.mark.req("face-exit-before-grounding:1")
def test_b2_contract_closeout_scope_verdict(client, rig):
    ctx = _face_context()
    ...
    for rep in range(1, REPETITIONS + 1):
        sid = f"r1b2-{rep}-{uuid.uuid4().hex[:8]}"
    ...
    assert all(counted), "有 repetition 未捕捉到 contract_closeout 的 brain 呼叫"
