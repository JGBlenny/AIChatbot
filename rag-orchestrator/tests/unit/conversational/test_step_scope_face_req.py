...
async def test_scope_switch_passed_through():
    opt = _opt({"action": "ask", "next_question": "q", "extracted_fields": {}, "scope": "switch"})
    r = await opt.conversational_step("RULES", "SYS", {"collected_fields": {}}, "我帳單怎麼繳")
    assert r["scope"] == "switch"


# ── scope 缺省 → 'stay'（向後相容）──
@pytest.mark.req("mid-session-switch:3.1")
async def test_scope_defaults_to_stay():
    opt = _opt({"action": "converge", "converge_kind": "answer", "extracted_fields": {}})
    r = await opt.conversational_step("RULES", "SYS", {"collected_fields": {}}, "查我合約 678")
    assert r["scope"] == "stay"


# ── scope 越界值 → 正規化為 'stay'（防越界）──
@pytest.mark.req("mid-session-switch:3.1")
async def test_scope_invalid_normalized_to_stay():
...
# ── face 原樣帶出（引擎再驗證面向集合）──
@pytest.mark.req("mid-session-switch:3.1")
async def test_face_passed_through():
    opt = _opt({"action": "converge", "converge_kind": "answer", "extracted_fields": {}, "face": "違約金"})
    r = await opt.conversational_step("RULES", "SYS", {"collected_fields": {}}, "提前解約違約金怎算")
    assert r["face"] == "違約金"


# ── 有 faces 清單 → 注入 prompt（供 brain 選面向）──
@pytest.mark.req("mid-session-switch:3.1")
async def test_faces_injected_into_prompt():
    opt = _opt({"action": "ask", "next_question": "q", "extracted_fields": {}})
    await opt.conversational_step("RULES", "SYS", {"collected_fields": {}}, "x", faces=["狀態判斷", "違約金"])
