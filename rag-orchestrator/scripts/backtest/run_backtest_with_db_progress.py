...
async def main():
    ...
    # 排序並添加分批參數
    # BACKTEST_SAMPLE_SEED：設定後改用種子化偽隨機排序（可重現的抽樣），
    # 避免 ORDER BY id 抓到 imported 同義改寫題連號區造成主題塌縮
    sample_seed = os.getenv('BACKTEST_SAMPLE_SEED', '').strip()
    if sample_seed:
        query += " ORDER BY md5(id::text || %s)"
