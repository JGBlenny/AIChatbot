...
| DSP-008 | ⚠️爭議:JGB 到底哪些資料能批次匯入（物件／租客／帳單／合約）——jgb2 查證與 chatai 知識庫互相矛盾；未裁前 P1-2／P1-3 的『答錯』定性與知識修正方向都不能定 | 代理 | docs/presales-assistant-quality-audit-20260902.md §P1-3（引 jgb2 路由查證：只有帳單支援批次匯入，物件／合約／租客都要手動建） ／ 實作:knowledge_base 3357「物件 批次匯入 上傳」(target_user NULL)、3431「物件批次匯入 Excel 建立」、3476「租客批次匯入 Excel」、3404「大量建立帳單 Excel 批次匯入」皆 is_active；系統脈絡 3798 功能索引亦列「租客批次匯入」。本機 :8100 prospect 問「舊系統的資料能匯進來嗎」即以 3357 原文作答 |  |  |
