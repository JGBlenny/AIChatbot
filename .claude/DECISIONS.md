...
| DSP-012 | ⚠️爭議（未裁）：agentic-mcp requirements R11.1「知識文字視為不可信輸入，⛔ 不得把回傳文字拼進 system prompt」vs R5.1「售前池整理成大綱進上下文（system prompt）」字面衝突。design 1.2 假設：大綱是 server 端由已審核知識列組裝、非工具回傳，套 nonce 分隔標記後允許；知識入庫（knowledge_import／document_converter）作者面仍是注入面。待業主裁：接受此假設（改 R11.1 措辭）或大綱改走 `kb.get("outline:*")` 按需讀取不進 system prompt | 代理（security-reviewer 第二輪 2026-09-04） | .kiro/specs/agentic-mcp-orchestration/requirements.md R5.1／R11.1；design.md 1.2 元件 5、決策 14；⚠️ 落點段落＝design 1.2 元件 5 | .kiro/specs/agentic-mcp-orchestration/design.md |  |
