...
1 intake        腳本      讀輸入、凍結 sha、產「受測物定義清單」骨架
   ↓
2 structure     Workflow  G0/G2 判者（3 角度：提問路徑／內容邊界／受眾層級）→ 合成
   ↓                      → steps/02-structure.md（Workflow 判者，1.4 實作）
3 phrasing      腳本+人審 講法／缺口格掛回細目；相似細目待審清單
   ↓
4 answerability Workflow  每格 2 判者 fan-out、不一致加第 3
   ↓                      → steps/04-answerability.md（Workflow 判者，1.4 實作）
...
| 2 | [`steps/02-structure.md`](steps/02-structure.md) | intake.json | `schemas/structure-proposal.json` | Workflow 判者（1.4 實作） |
| 3 | [`steps/03-phrasing.md`](steps/03-phrasing.md) | structure-proposal.json | `schemas/phrasing-map.json` | 腳本＋人審 |
| 4 | [`steps/04-answerability.md`](steps/04-answerability.md) | phrasing-map.json | `schemas/answerability.json` | Workflow 判者（1.4 實作） |
