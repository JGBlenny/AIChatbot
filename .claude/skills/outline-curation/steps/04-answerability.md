...
- **收尾**：`../scripts/finalize_answerability.py`——把判者結果（`answerability_judge.py` 輸出或合併後結果）包成 StepEnvelope（過 `../schemas/answerability.json`）、寫 journal 檔、更新狀態檔 `answerability` 鍵；`needs_rubric_revision=true` 時仍寫檔但 exit 2。
