...
- [ ] (❓) 2026-09-02 **DSP-002（已重寫）**：「b2b 可見」要用哪一個數字報 — 三選一：**(a) 答題母體 293**（`_B2B`，過濾後∧answer 非空，⛔ 不模型任何檢索路）／**(b) 兩路聯集 354**／**(c) 分路報 向量 354／詞面 320**。⚠️ 原版問成「`_B2B` vs `visibility()` 二選一」且引用「向量 293／詞面 264」——**兩者都錯**：`visibility()` 已能分路報（實測 354/320，純 SQL 與實跑逐筆一致），做不到的只有 `_B2B`。⚠️ 即使選 (c) 也 ⛔ 不得宣稱「這一題撈得到」——`visibility()` 自陳只給**上界**（詞面路的逐題 `keywords && 問句斷詞` 它不模型）。⚠️ 連帶：`visibility()` 住 `backtest/`，立為營運正本需先升格
