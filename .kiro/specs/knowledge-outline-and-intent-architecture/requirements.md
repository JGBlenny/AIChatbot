...
> 建立時間：2026-09-06　階段：requirements-approved（**v2**，依 fresh 審查 A1–A14／B1–B10／C1–C8 修訂；業主 2026-09-06 核可，R1.5 依核可時定向改寫；**v2.1 同日 M-a 試作後修訂 R1.5 執行形態**）　語言：zh-TW
...
| 15 | 售前不開 D6 抽取式（`PRESALES_EXTRACTIVE`；此 D6 為既有旗標代號，≠ 本 spec 待裁 D 系列），先補知識 | 業主 2026-09-04 |
...
- R1.5 skill 執行形態 SHALL 以 hook 承接流程紀律（R1.4 四道閘門）、以 Workflow 承接固定順序步驟與獨立判者 fan-out（業主 2026-09-06 核可時定向：本 repo 現無 hooks／workflows 先例係刻意保留，由本 spec 建立）；設計階段 SHALL 以最小試作（先做「判者 fan-out」一步）量可重跑、每步可稽核、判者隔離（判者不得見系統判定）、成本與時間、失敗可從哪一步續跑；⛔ 試作結果只調參數與切分，不回頭改形態。
  **v2.1 修訂（2026-09-06，M-a 試作後業主裁）**：試作量出 Workflow 子代理在本機（8 GB）撐不住且判者間不共享快取（44 格 $15.84）。執行形態改為：判者 fan-out＝腳本直打模型 API（獨立請求＝判者隔離、strict json_schema＝schema 強制、journal 續跑）；結構提議＝Claude Code 子代理（4 次呼叫）；Workflow 腳本留檔作參考、不再是執行路徑。R1.5 其餘要求（可重跑、每步可稽核、判者隔離、成本可量、可續跑）不變且已由 1.5／1.6 試作證明。
