...
| 接線 | 位置 | 內容 |
|---|---|---|
| 候選注入 | `run_turn`：`outline = agent_state.get("outline")` 之後 | `sel = await selector.select(doc, identity, q)`；`outline = CandidateOutlineDoc.from_selection(doc_outline, sel, toc)` 或降級 `from_visible(可見細目全集, toc)`（**1.16 修訂：不是未過濾整份**）＋`violations.append("candidate_fallback_full_outline")`；同一物件餵 `_seed_outline_provenance` 與 `build_messages` |
| 身分派生 | `_slots_for_prompt(state)` 之後 | **強制覆寫** `slots["identity"] = identity.resolved_audience()`（F9：⛔ 不用 `setdefault`——DB 攤平在前，setdefault 會讓模型寫的值勝出）；`identity_source ∈ {entry, anonymous}`（**1.17 修訂**：有 `role_id` 或 `user_id` 為 entry、否則 anonymous，⛔ 不看 `vendor_id`——jgb2 契約 prospect 不帶雙證且 vendor_id 由 AI 側解；F10）；⛔ 不寫 DB。**`identity` ⛔ 不在 `SlotKey` 內**，模型寫不到；模型可寫的子身分用另一鍵 `identity_detail`。契約測試：模型經 `slots_set` 寫入任何值後，prompt 的 `identity` 仍為 entry 派生值 |
...
| 日期 | 版本 | 變更內容 | 修改者 |
|---|---|---|---|
| 2026-09-07 | 1.19 | 4.4 依 (a′) 縮窄收案（DSP-036）：步 2 探針改以 DSP-035 基線逐字 3 rep＋H4 盲標收案——候選層「通過」（gold 前五 0.906、零故障）、回答層「已知水準」（答到 40.8%、首拒 29.3%、無據 1.15% 兩判者 87/87 一致、敏感 0 漏）；**翻轉率（≥5/10）與 `budget_exhausted` 門不再作 4.4 收案門**（僅觀察）；5.1 定義版隨之收；`agent_eval` JSONL／report **無 model 欄位**（受測組態只能靠容器 env 佐證，列補強項）；demo 目標改為 LINE OA 經 `/mcp` `agent.turn`（prospect），③④⑤ LIFF 線先不併（業主 2026-09-07 裁） | 業主／AI |
...
| 2026-09-07 | 1.16 | M-d S1（4.1）收案：`CandidateOutlineDoc`（pydantic，`from_selection`／`from_visible`，audience／version／sha 複製自母 doc）；`run_turn` 於 `outline` 取得後 `_select_outline`，同一物件餵 `_seed_outline_provenance` 與 `build_messages`；**降級＝可見細目全集＋toc**（非未過濾整份；3.2 P3 債在此還清）；toc 每回合依身分 `build_canon_toc`；`miss_kind` 值域 `{hit,none_visible,no_candidate,index_unavailable,None}`，`None`＝無候選機制（`--candidates off`／受眾無正本）；violations 三互斥：`candidate_fallback_full_outline`（selector 回 None）／`candidate_selector_error`（例外）／`candidate_none_visible`（可見子集空，可觀測非故障）；trace 白名單 13→16（票 C）＋id 形狀守門 fail-closed `candidate_ids_shape_invalid`；影子／`trace_view` 鏡射；主／影子共用同一 selector；啟動 `prepare` 硬上限 `PREPARE_TOTAL_TIMEOUT_S=60`，逾時索引留 `absent`、不掛啟動；health：`mcp_facade.agent_configured()`（判準逐位元搬自 `app._agent_configured`）且 prospect 索引非 ready ⇒ 紅（3.3「不致紅」限於未接線期）；政策文 `kb.get` 禁句改為允許句＋`outline:` 前綴形狀（目錄行為裸 fine id、`kb_get` 只認前綴）。**待裁已裁**：b2c 模式 prospect 入口正規化為 b2b 歸 S2（4.2，先 security-reviewer）。P4 留：形狀違規在 `from_selection` 即以 `candidate_selector_error` 收掉；降級建構子自身 raise 會外拋（無可達輸入）；啟動列印例外訊息全文 | 業主／AI |
| 2026-09-07 | 1.17 | M-d S3（4.3）＋S2a（4.2 程式面）收案：`agent_eval --candidates on/off`（函式層預設 off、CLI 預設 on）、`--set outline-probe` 佔位、JSONL 28 鍵、`EvalRecord.candidate_violations` 承載報表三計數、真 provider 順序 key 檢查→索引→就緒閘門 exit 5→`build_real_runtime(candidate_selector=)`；**入口正規化**：`identity.normalize_entry_mode`（prospect 一律 b2b，含明送 b2c；REST `VendorChatRequest.mode` 預設 `b2c` 使原補值不觸發，故兩入口都呼叫）＋`_STATS.identity_mode_normalized` 三處觀測不致紅；**`identity_source` 依 jgb2 契約形狀**（有 `role_id`／`user_id` ⇒ entry，否則 anonymous，⛔ 不看 vendor_id；元件 7 原文「有 role_id／vendor_id 為 entry」作廢）；F9 三守門（`_slots_for_prompt` deny 派生鍵、`write_slot` 驗 enum、派生鍵不入 `SlotKey`）；`SlotKey` 十值；F10 AST taint 掃描器（18 種 sink 形狀正對照）；政策文一句定義（⛔ 無例子）。資料範圍變更（F-2）：prospect 走 b2b 謂詞 ⇒ 多見全域 `system_provider` 池、少見業態 NULL／`all_users` 列，影響所有 MCP 工具；跨業者隔離不變。已知債：新四鍵個資最小化；`fallback_old_chain` 後舊鏈仍重問。S2b（句型表常數＋R4.2 測試）待業主核句型表 | 業主／AI |
| 2026-09-07 | 1.18 | M-d 完成（S2b 收案）：「身分反問句型表」定性＝**測試用判定清單**（`presales_gate.IDENTITY_REASK_PATTERNS`，五欄×6、唯讀、⛔ 不進 prompt，測試側掃描守），不是檢索或輸出過濾；R4.2 契約測試的否定斷言**只套 identity 欄＋槽位已知欄**（業主定案：入口已帶身分者不得再問身分，但未知的戶數／痛點／團隊／功能偏好可依售前規則補問，一次一題）；四欄其餘句型供 anonymous 正例與 4.4b 對真輸出量「一次一題、已知不重問」；誤殺以「一類」治理、先留著量。單元層以假模型只證尺／定義句／掃描器，真模型行為留 4.4b | 業主／AI |
