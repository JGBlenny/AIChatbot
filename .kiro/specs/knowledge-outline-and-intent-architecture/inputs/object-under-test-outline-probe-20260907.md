# 受測物定義清單——步 2 探針（任務 4.4，`agent_eval.py --set outline-probe`）

> 依 tasks 4.4a／R6.4／design 驗收邏輯總表列 2。**核可欄非空後才准起 4.4b**（真模型、打 API）。
...
| 對話規則 `services/agent/agent_rules.py` | sha256 `5c14ec238ba2d238…`（含 4.1 的 `kb.get` 允許句、4.2 的身分定義句；5.1 三段定義**尚未**搬遷） | `shasum -a 256 rag-orchestrator/services/agent/agent_rules.py` |
