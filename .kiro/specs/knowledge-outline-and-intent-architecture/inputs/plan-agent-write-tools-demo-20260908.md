...
| **W6** 口語穩定度（業主 2026-09-08：「完成度需要能夠應付真實用戶操作」，排進 Plan） | **先量再修、修在規則層**。(a) 量：40–60 回合口語壓力劇本（換話題、確認卡中途換題、反問、收尾語「好了／謝謝／先這樣」、錯字／簡稱、一句兩意圖、同戶跨類別、追問「為什麼」），pm 經 `/mcp`，替身後端，跑**兩輪**取變異；雙判者盲評（沿用探針 55 H4 brief）；指標＝**可答題轉人率／資料事實答錯率／有明確 ref 仍反問率／p50 延遲**，先出基線。(b) 修（⛔ 不寫特例、⛔ 提示詞不列例子）：收尾語＝封閉類別的定義句與對應 `kind`（短收尾、不轉人）；轉人固定句依受眾（pm 用「轉專人」、非售前「找真人」）；「有明確 ref 仍反問」走 ask 政策定義；Verifier 對工具事實引用的比對是否過嚴（`QUOTE_NOT_COVERING`）以數字判、只動尺不動答案。(c) 延遲：line-bot 契約加 typing 指示；模型 low→medium 以「每回合 +N s 對拒答率 −M%」的實測數字裁。 | 劇本（scratchpad→`inputs/`）、`services/agent/agent_rules.py`（定義句）、`conversational_config.py`（受眾固定句）、`verifier.py`（尺，需數字）、line-bot 契約 | **通過標準先定再跑（待裁，建議）**：可答題轉人率 ≤20%、資料事實答錯 0、明確 ref 反問率 ≤10%、p50 ≤20 s；修後重跑同劇本，三率不升、答錯 0；plan-verifier 對 W6 單獨 READY 後才派 | 主 session（量）＋executor（規則層）→ verifier |
