...
| S9-6 | P1 | >3 張三方矛盾（契約拒／L3-F 收 5／碼截 3） | **業主裁 2026-09-08「可以超過三張」**：上限 5、第 6 張起 `INVALID_INPUT`；`analyze_images` 的 `[:3]` 截斷對 MCP 路徑移除；契約列改寫；L3-F 的「JGB 單照片數＝5」為非目標（`repair_create` 不送照片） | (2)、W-D |
...
| S9-21 | P1 | 記憶體／DoS（3×5 MB＋base64＋請求副本） | FIX：串流中止＋抓回即縮 1024px 再 base64；併發上界＝`RATE_PER_MIN` 明列 | (2) |
...
**W8 (2) 驗收（確定性，各附正對照；`tests/unit/agent/test_image_entry_req.py` 假抓檔器＋假辨識器）**：(i) 白名單預設值釘住；白名單外主機／http／IP／userinfo／非 443／3xx／超 5,000,000 bytes（串流中止、抓檔器呼叫 1 次、辨識器 0 次）／`exp` 過期／非圖片 magic bytes ⇒ 該張丟棄且辨識器 0 次；正對照＝合法網址 ⇒ 抓檔 1 次、辨識 1 次；(ii) 6 張 ⇒ `INVALID_INPUT`、抓檔器 0 次；5 張 ⇒ 5 次抓檔、辨識收 5 張（⛔ 不截 3）；(iii) 帶 GPS EXIF 的測試圖 ⇒ 送辨識的 bytes 無 EXIF、尺寸 ≤1024（正對照＝原檔有 EXIF）；(iv) 逾時兩案（同 W7 (i)(ii)）；(v) 配額用罄 ⇒ `RATE_LIMITED`、抓檔 0 次；(vi) 辨識回不在樹的分類 ⇒ 卡上 `UNSPECIFIED_CATEGORY_ZH`；回 `suggested_emergency=2` 而使用者未表明 ⇒ 卡上「非緊急」（vision 建議不傳播）；回自由文字 description ⇒ 卡上「（未填寫）」、payload description 空；(vii) `damage_visible=false` ⇒ facts 含「看不出損壞」、卡描述留空；信心低 ⇒ `quick_replies` label ⊆ 分類樹節點名；(viii) 特徵 bytes／base64 片段跑完回合不出現在 trace 快照、`usage_events`、`form_sessions` 列、caplog（正對照＝answer 出現分類名）；辨識例外訊息含特徵字串 ⇒ log 只有例外類別名；(ix) `select_scope` 指 A 戶時帶圖回合對 B 戶 ⇒ 不出卡（正對照＝無 scope 照常出卡）；(x) vision 失敗 ⇒ violation `image_recognition_failed`、健檢旗、固定句（⛔ 不靜默）；(xi) 不帶 `image_urls` ⇒ 回應逐位元同現行；`make audit` 不變量 27–31 PASS。**真線路一次**（`RUN_REAL_OPENAI=1`；素材＝scratchpad `photos/` 六張，⛔ 不進 repo）：leak／breaker／lock／mold 各出對應分類卡、plain ⇒「看不出損壞」、two_damages ⇒ 一張卡＋候選；gpt-4o 與 gpt-5-mini 各跑一輪並排，費用 ≤ US$0.10。**主驗收**：LIFF L3-C／D／F／R7b／P 五案經 `/mcp` 實跑盲判。
