...
> 狀態：**5.1 單元收案（2026-09-07）**：executor＋fresh verifier CONFIRMED（附錄 A 逐字＝`policy_provider(prospect)`、四鐵則逐字＝HEAD、1,511／8、唯一整條刪除＝`至少要有一筆 refs`、紅前綠後控制、三 persona 150／130／108、pm／tenant 變體正確；unit 1312、audit PASS）；不變量 34 查驗器補 `outline-probe` 形狀（49 題 ∩ 167 講法＝∅）。**主驗收（§1.3 重跑）已跑：`inputs/probe-report-51-20260907.md`——三門未過（翻轉 3/10、budget_exhausted 22、邊界 6/15），四門過（敏感誤判 40、對照組 0、敏感 0 漏、H4 5.6%↓）；停下交業主。**前史：收尾審查 READY → 業主核准（2026-09-07，§5 六項照建議）→ executor（第 3 稿；第 2 輪 REVISE 1 P2＋2 P3 FIX：邊界硬答門檻、pm／tenant 身分句斷言、刪除措辭）（第 1 輪 REVISE 7 條皆 FIX：定義文實寫進附錄 A 並實測 1,511 字、上限改為實測值；四條鐵則逐字凍結＋測試（裁定 16）；補列第三條既有斷言並保留該句；「不重複」斷言改為刪除後才成立的字面；4.1／4.2 句逐字不動；H4 基線改「附標題封包 vs 已扣盲點 3/34」；rollback 與流量暴露；pm／tenant 政策文去補問段；H2 門檻對齊凍結值 ≥2；成本兩臂理由；預登記「答到但引用不合法」分項；`other` 定義依來源）。核准前 ⛔ 不寫碼。對應 tasks 5.1、R4.4／R4.7、design 元件 7、裁定 13／16。**起因**：4.4b（`inputs/probe-report-20260907.md`）候選層過、回答層未過（A 層 22/30 回合有對的細目仍一次嘗試都不做就轉人）。**驗收尺＝同一凍結題集重跑**（`eval/outline-probe-20260907.json`，sha `22aa2c10…`，⛔ 不改題）。
