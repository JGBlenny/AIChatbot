...
| 模型 | **gpt-5-mini＋`reasoning_effort=minimal`**（`AGENT_MODEL=gpt-5-mini`、`AGENT_REASONING_EFFORT=minimal`；runtime sha 見 git）；Verifier 現行尺（DSP-029a ratio 0.5 ∧ 下限 4 ∧ 全極性，DSP-034 後） | `git log -1 -- rag-orchestrator/services/agent/verifier_rules*` |
