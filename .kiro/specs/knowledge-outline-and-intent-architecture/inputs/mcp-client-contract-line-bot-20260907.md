...
| **語音（W7，2026-09-08 業主開）** | 請求 `agent.turn` 加選填 `audio_urls: string[]`（≤3 段、每段 ≤60 秒／≤5 MB、**只收 `https://relay.jgbsmart.com/...` 簽章 URL**、`exp` 未過期）；`message` 可為空字串。回應加選填 `transcript`（轉錄文字，LINE 端請先回「我聽到的是：…」再顯示答案／確認卡）。錯誤碼：`AUDIO_NOT_SUPPORTED`（服務端 `STT_PROVIDER=off`）、`AUDIO_URL_NOT_ALLOWED`、`AUDIO_TOO_LARGE`、`AUDIO_TOO_LONG`、`AUDIO_EXPIRED`、`AUDIO_FETCH_FAILED`、`STT_FAILED`。轉錄模型 `gpt-4o-mini-transcribe`（`language=zh`，輸出轉繁體）。⛔ 音訊不落地、不進日誌；**`transcript` 與 `answer` 同級：LINE 端 ⛔ 不入 log、不落 DB**；`exp` 只做過期預檢、**`sig` 驗簽演算法與密鑰交換待 line-bot 定**（可選：簽章 URL 帶 `dur` 秒數供服務端驗 ≤60） | Plan W7；`speech_to_text.py` |
