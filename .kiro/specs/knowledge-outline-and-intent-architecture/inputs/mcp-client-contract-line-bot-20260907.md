...
| **回合結果 `outcome`（DSP-043，2026-09-08 業主裁）** | 回應第七鍵，永遠有值：`{"state": …, "expects": …, "action": …, "ref": …}`。`state`：`answered`（一般回答，含清單直答／查無此筆）、`clarifying`（反問或請選分類）、`confirm_pending`（出了確認卡）、`confirmed`（建單／延期成功；重按同鍵仍回 confirmed）、`cancelled`（按取消／修改）、`failed`（寫入失敗、確認已失效、照片處理失敗／逾時）、`handoff`（轉專人）、`out_of_scope`（點選後問別戶或別戶寫入被擋）。`expects`：`text`／`choice`（`quick_replies` 是分類名）／`button`（三顆確認鍵）／`none`。`action`：`repair_create`／`bill_due_extend`／null。`ref`：成功後 `{"type": "repair"|"bill"|"contract", "id": "12346"}` 或 null。**呼叫端只看這個物件決定畫面**（LIFF 收畫面＝`state==confirmed`），⛔ 不解析 `answer` 字串。 | `mcp_facade.py` `TurnOutcome`；`runtime.py` `OUTCOME_STATES` |
| **LINE 照片緩衝與 LIFF 入口（2026-09-08 業主裁）** | line-bot 把同一使用者 **3 秒內**的照片（與文字）合成一次 `agent.turn`（`image_urls` ≤10，`message` 可空），⛔ 不拆成兩個回合；3 秒後才到的算下一回合。**LIFF 六格一律走 MCP**：LIFF 前端 → line-bot relay 後端 → 同一把 key 打 `agent.turn`（文字、`select:<type>:<id>`、`image_urls` 同一條）；⛔ 不接舊鏈 REST `/rag-api/v1/message`（無寫入、無點選、無本次正本），不另開 HTTP 端點。 | 帳本 §1b R10／R10-c；DSP-043 |
| **照片進場（W8 (2)，2026-09-08 line-bot 契約＋業主裁）** | line-bot 自行從 LINE 抓檔、落 relay 受控附件（依租戶分目錄、寫入時計數超限即失敗刪殘檔），把 **HMAC 簽章網址**放進 `agent.turn` 的 `image_urls`（簽章涵蓋檔案 id＋到期，⛔ 不可挪用、不可延期）；chatai **自行下載**（白名單 `https://relay.jgbsmart.com` 等值、不跟轉址、每張 ≤5,000,000 bytes 串流硬閘、`exp` 時戳預檢、內容型別驗證、縮圖去 EXIF 後才交辨識）。**張數上限 10**（業主 2026-09-08「可以超過三張」「超過五張分批送」：一次全送同一回合，chatai 內部每 5 張一批辨識、預算不夠時明講只看了前 N 張；⛔ line-bot 不要拆成兩個回合；第 11 張起整回合 `INVALID_INPUT`——⛔ 不是 422，/mcp 工具錯誤只有封閉五值）；網址有效期 900 s（上限 3600）。**`message` 可為空**（只傳照片；兩者皆空才 `INVALID_INPUT`）。辨識信心低時會先回一個選分類的回合：`quick_replies` 為分類名（`label`＝`value`＝分類名，≤3 顆，沒有確認鍵），使用者點了就把該字串原樣送回。辨識結果**不回顯**給 line-bot：只會反映在確認卡（分類、急迫）與答案文字；照片 bytes 不落地、不進任何紀錄。已知取捨：簽章不綁租戶（line-bot 為唯一呼叫端）；照片經 API 送 OpenAI。 | Plan W8 (2)；`services/agent/image_fetch.py`（待落地） |
| **清單點選（W8，2026-09-08 雙方同意；DSP-042 同日裁）** | 使用者在 LINE 點清單那一筆時，line-bot 把快速回覆 value **原字串**當 `message` 送入：`select:<type>:<id>`，`type ∈ bill|contract|repair`（**第一版**；`estate`／`meter` 待工具層 ref 語義補齊後再開）、`id` 為 JGB 編號（`^[A-Za-z0-9_-]{1,32}$`）。服務端在模型前以封閉正則整句攔截，走既有查詢工具（**以呼叫者 `role_id`／可見範圍收口**：不在範圍或不存在 ⇒ 一律「查無此筆」，⛔ 不洩存在性）；命中 ⇒ 回該筆事實（`kind=answer`，程式產出）並記入會話，之後追問用文字即可。`select:repair:` 服務端支援（修繕進度），line-bot 第一版可不渲染。**已知取捨**：真人可手打此格式，最壞情況＝查到**呼叫者 role 範圍內**（含同 role 其他租客）的資料——與現行 pm 單證讀路徑同級；速率：全體使用者共用 key 的每分 60／每小時 120 次。⛔ 不要把 id 寫進自然句（「幫我查 id:123」）——那會回到讓模型抽 id 的路 | Plan W8 (1)；`runtime.py` 清單點選段（待落地） |
| **語音（W7，2026-09-08 業主開）** | 請求 `agent.turn` 加選填 `audio_urls: string[]`（≤3 段、每段 ≤5 MB（硬閘）、每段 ≤60 秒（**best-effort：只在簽章 URL 帶 `dur` 秒數時驗**）、**只收 `https://relay.jgbsmart.com/...` 簽章 URL**、`exp` 未過期）；`message` 可為空字串。回應加選填 `transcript`（轉錄文字，LINE 端請先回「我聽到的是：…」再顯示答案／確認卡）。錯誤碼：`AUDIO_NOT_SUPPORTED`（服務端 `STT_PROVIDER=off`）、`AUDIO_URL_NOT_ALLOWED`、`AUDIO_TOO_MANY`（>3 段）、`AUDIO_TOO_LARGE`（>5 MB）、`AUDIO_TOO_LONG`（僅 `dur`>60 時）、`AUDIO_EXPIRED`（`exp` 過期）、`AUDIO_QUOTA_EXCEEDED`（每 key 每小時 bytes 配額）、`AUDIO_FETCH_FAILED`、`STT_TIMEOUT`、`STT_FAILED`。轉錄模型 `gpt-4o-mini-transcribe`（`language=zh`，輸出轉繁體）。⛔ 音訊不落地、不進日誌；**`transcript` 與 `answer` 同級：LINE 端 ⛔ 不入 log、不落 DB**；`exp` 只做過期預檢、**`sig` 驗簽演算法與密鑰交換待 line-bot 定**（可選：簽章 URL 帶 `dur` 秒數供服務端驗 ≤60） | Plan W7；`speech_to_text.py` |
| `kind` 值域 | 依 runtime 輸出契約（`answer`／`ask`／`handoff`…）；**待核**：demo 前以 `trace_id` 對 `/api/v1/agent/trace/{id}` 抽樣確認 | `services/agent/output_schema.py` |
| `quick_replies[]` 形狀 | 一般回合：現行 runtime 輸出（實跑為準）。**確認回合（寫入工具，DSP-038）**：`[{"label": "✅ 確認送出", "value": "confirm_submit:<pending_id>"}, {"label": "✏️ 我要修改", "value": "confirm_edit:<pending_id>"}, {"label": "❌ 取消", "value": "confirm_cancel:<pending_id>"}]`（label 實值含 emoji 前綴，沿用 `conversational_engine._DEFAULT_QR_LABELS`；顯示照用即可）——按鈕送出時把 `value` **原字串**當下一回合 `message` 送回（⛔ 不改寫、不加字）；`answer` 即確認卡文字 | `output_schema.py` `quick_replies`；Plan W2／W3 |
| `handoff` | 轉人時非空，含 `handoff_reason`（如 `sensitive_no_grounding`、`budget_exhausted`、`no_grounding`）；呼叫端只要「非 null ⇒ 顯示固定轉人句＋提供真人入口」 | `output_schema.py` `handoff_reason` |
