...
| **清單點選（W8，2026-09-08 雙方同意）** | 使用者在 LINE 點清單那一筆時，line-bot 把快速回覆 value **原字串**當 `message` 送入：`select:<type>:<id>`，`type ∈ bill|contract|meter|estate|repair`、`id` 為 JGB 編號（`^[A-Za-z0-9_-]{1,32}$`）。服務端在模型前以封閉正則整句攔截，走既有查詢工具（**以呼叫者 `role_id`／可見範圍收口**：不在範圍或不存在 ⇒ 一律「查無此筆」，⛔ 不洩存在性）；命中 ⇒ 回該筆事實（`kind=answer`，程式產出）並記入會話，之後追問用文字即可。`select:repair:` 服務端支援（修繕進度），line-bot 第一版可不渲染。**已知取捨**：真人可手打此格式，最壞情況＝查到自己權限內資料（與 `confirm_submit:` 同級）。⛔ 不要把 id 寫進自然句（「幫我查 id:123」）——那會回到讓模型抽 id 的路 | Plan W8 (1)；`runtime.py` 清單點選段（待落地） |
