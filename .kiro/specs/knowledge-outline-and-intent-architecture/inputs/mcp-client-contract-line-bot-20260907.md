...
| **照片進場（W8 (2)，2026-09-08 line-bot 契約＋業主裁）** | line-bot 自行從 LINE 抓檔、落 relay 受控附件（依租戶分目錄、寫入時計數超限即失敗刪殘檔），把 **HMAC 簽章網址**放進 `agent.turn` 的 `image_urls`（簽章涵蓋檔案 id＋到期，⛔ 不可挪用、不可延期）；chatai **自行下載**（白名單 `https://relay.jgbsmart.com` 等值、不跟轉址、每張 ≤5,000,000 bytes 串流硬閘、`exp` 時戳預檢、內容型別驗證、縮圖去 EXIF 後才交辨識）。**張數上限 5**（業主 2026-09-08「可以超過三張」；第 6 張起整回合 `INVALID_INPUT`——⛔ 不是 422，/mcp 工具錯誤只有封閉五值）；網址有效期 900 s（上限 3600）。辨識結果**不回顯**給 line-bot：只會反映在確認卡（分類、急迫）與答案文字；照片 bytes 不落地、不進任何紀錄。已知取捨：簽章不綁租戶（line-bot 為唯一呼叫端）；照片經 API 送 OpenAI。 | Plan W8 (2)；`services/agent/image_fetch.py`（待落地） |
