...
- 情境：走查 wt-bill#6「這戶除了這張還有沒有其他沒繳的」⇒ 直接列（不反問）或至多一次追問後「對」即列；restart 後「我剛剛問了什麼」⇒ 直答「這段剛開始」；3 輪 3/3。**宣稱縮小（2026-09-09 verifier r2 F3）**：修正後三輪為 2/3（r3 第 3 回合「對」後模型再問一次）——肯定語注入是程式決定性行為（單元 3/3），模型是否照做是模型層；第二批收案以「注入 3/3、模型 ≥ 2/3」計，殘餘記帳本、第三批後再量。
