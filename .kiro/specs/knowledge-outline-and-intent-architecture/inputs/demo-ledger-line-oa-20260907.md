...
## 1k. 第二批 T1–T4 落地與回測（2026-09-09 中午；HEAD `db4c42b9`；smoke-rag 線上同組態、每輪重起；劇本 `smoke/scenarios_batch2.json` 21 回合、計分 `smoke/b2_score.py`）

- 執行：T1（security-executor）`ask_target` 契約＋`entry_line`（執行代理 P1：`ASK_TARGETS` 缺售前對象 ⇒ 業主線裁定補 identity/scale/team/pain/interested、固定句改中性 `c4427a68`）；T4 措辭 `eb1d782e`；T2 兩出口 `0d5c9ac0`；T3 肯定語 `db4c42b9`。整合單元 3094 過。⚠️ smoke-rag 換碼後起不來一次：啟動自檢讀 `tests/fixtures/agent/known_*.json`，T1 改了它們、docker cp 漏掉——**這兩份 fixture 是啟動自檢的一部分，部署 build 會帶到，但 docker cp 熱換要一起 cp**。
- 第二批劇本三輪：11/13、11/13、12/13。穩定綠：進場句「信仰」不談宗教、「對」直接列、空會話直說、有前文判斷題不轉人（「要不要催他」→「建議催繳，系統可產草稿」）、全查無回查不到句、同戶第二件出卡（3/3，第一批時 1/3）、「嗎？。」0、內部 id 0。三輪都紅：「756248 你建議我怎麼做」反問編號類型——「只有名稱或編號先查」定義句模型不照做 ⇒ 第三批做程式前置查詢。r2 一個紅：「要不要催他」被模型自報 `sensitive_no_grounding` 轉人——敏感類不降級是設計，但模型有「什麼都說敏感」的逃生門 ⇒ 第三批程式側對問句判敏感、不信自報。
- 走查 24 回合：**12/12**（第一批最佳 10/12）；變形集 22 回合：轉人 0；線③：12/12、外洩 0、轉人 0、p50 6.6 s／p95 13.1 s。
...
## 1n. line-bot 第二輪走查驗收（2026-09-09 16:34；他們 Playwright 開 LIFF 打線上 MCP、每回合帶開場句當 `context`、39 屏；scratchpad `linebot_acceptance_20260909b.txt`）

| 第一輪問題 | 第二輪 | 他們裁決 |
|---|---|---|
| H1 延三天寫入過去日期 | 不寫入了、改問要改到哪天；但句子仍先算出 9/04 再問 | 半好：擋了寫入、算法沒修 |
| H3 建單後單號答不出 | 「剛剛建立的修繕單號是 12346」 | 好了 |
| H4 卡收掉仍教按「我要修改」／第二件當改單 | 「已送出無法在對話中修改」；第二件判成同戶新單 12347 | 好了（改描述沒給出路，半分） |
| H2 開放題罐頭轉人 | 0 次轉人（第一輪 8 次）；但「要不要打電話」「上次報修修好了沒」變成「想處理哪一件事？」——上下文全丟 | 變形：轉人→裝沒聽到 |
| H5 過度確認 | 「其他未繳」一次答還列 4 筆；restart 後不迴圈；但把 `context` 開場白當使用者問的 | 好了一半 |
| 介面三件 | 已上線；這輪 0 轉人所以「找真人」鈕沒驗到 | 清空 |

**新冒出（第四批輸入）**：(a) **未結單數字矛盾**——確認卡說「另有未結單 1 張（8591）」，三步後「這戶還有沒有別的單」答「只有剛建立的 12346」：模型拿 S2 記憶段（本對話完成的動作）當該戶全部紀錄、沒查修繕單（線上 log：該時段 15 回合 `tool_calls=0`）。(b) **第一句逾時 60 s**（線上 log 一筆 `TOOL_TIMEOUT`；冷啟或改寫迴圈撞 45 s 預算）。(c) H1 算法：「延三天」仍以原到期日為基準算出過去日期再問。(d) `context` 開場白被當成使用者的話（restart 後「我剛剛問了什麼」答「想看哪一戶？」）。(e) 物件名單獨講一次不認、追問舉「A棟302」這種本帳號沒有的例子；改描述沒指路；回聲確認一次；帳單事實夾「可取消（收回）／手動標記到帳」工程語。
line-bot 側：拍照鍵每回合保留（部署中）；「建立帳單」格用「建立」的框回「看」的問題（格名與能力不對稱，待定）。

## 1o. 第四批 V1–V4 落地（2026-09-09 傍晚；程式 HEAD `c14574e3`）

- V1 記憶段標題「本對話裡建立或修改過的…（只是這段對話做過的事，⛔ 不是該戶的全部紀錄）」`a1222c9a`；V4 政策三句（該戶有哪些單一律查／畫面提示不是使用者的話／追問不編造範例）`7f54b045`；V2 最近編號不可引用資料段（釘住範圍不注入）＋零查詢轉人有前文時迴圈內改寫一次、`context` 前綴「畫面提示（…不是使用者說的話）：」、空會話句改寫 `46e21ff0`；V3 延 N 天以原到期日或今天較晚者起算、render 關鍵字 `today`（payload 帶 `today` 鍵一律 INVALID_INPUT）、三個呼叫點同一時鐘、兌現端跨午夜 today−1、卡印「起算日」`c14574e3`。V5 runbook §20-3b 暖機 `2b25dd22`。整合單元 3330 過。
- V3 附帶：出卡端 S1 閘對 `bill_due_extend` 結構上到不了（基準 ≥ 今天且 days ≥ 1 ⇒ 新到期日必在今天之後），閘保留在兌現端與未來屬性；卡文字多一行 ⇒ 部署後 TTL 600 s 內舊卡兌現會 `CONFIRMATION_REQUIRED_TEXT`（自清）。
- 回測第一輪（HEAD `c14574e3`）：9/13 ×3，兩個一致失分＋2 回合 `budget_exhausted` 轉人，根因兩個、都在機制層：① `tools/action.py` 的 S1 句「不得早於今天」與 V3 衝突，模型改口反問「要改到哪一天」不出卡 → 描述改「起算日加天數必在今天之後、⛔ 不反問、直接提出確認」；② pair 極性裸狀態詞錨定撞複合詞（「待繳費」×「繳費期限」5 次誤殺）→ 錨定改「對面要有已＋狀態詞」`7cb9c636`（rules 1.4.1、56 對；「待發送」vs「尚未發送」同極性不擋、vs「已發送」仍擋）。單元 agent+audit 1914。
- 回測第二輪（`7cb9c636`；lb2 ×3＋線③ ×1；`smoke/b4_run2/`）：**10／10／11 之 13、0 轉人**；阻擋判決 68 次嘗試只剩 1 筆 SENSITIVE_TOPIC（極性誤殺歸零）。線③ 48 回合 12/12 出卡、0 洩漏、0 轉人、p50 7.4 s／p95 16.8 s。失分皆模型層殘留：逾期題答「尚未逾期」3/3（模型引其他事實行、句級極性看不到，第五批候選 turn-level pair）、回聲確認式追問（V3 一輪「是否要我出示確認卡」、V1 一輪「您是要查…嗎」）、restart 一輪首回合 `TOOL_TIMEOUT` 60 s（第 4 會話、非冷啟；gpt-5-mini 偶發慢）。
- **部署 ①**（main＝feat＝`7cb9c636`；業主 push、我 pull／build／up；log `AGENT_VERIFIER_MODE=grounding_observe`＋startup complete）。公開 MCP 實打 lb2 21 回合 0 轉人：V3 卡「起算日 2026/09/09／新到期日 2026/09/12」✓、V1 含 8591 ✓、H4 已送出＋指路 ✓、restart「還沒說過話」✓、暖機首回合 5.9 s。**抓到第四批自帶的回歸（F2）**：conv-2／conv-3／restart-3 答「收到，我會把本對話最近出現的編號當成那筆…」——V2 改寫定義句以 `role=user` 回模型，模型把它當使用者指令複誦、原題沒答（本地 r2 conv-3 也有一例，`b4_score.py` 的正則看不到這型病灶——只擋反問句）。
- **F2 修**（`e0fbc6e4`，契約層通用）：迴圈內四個回模型改寫點（SCHEMA／VERIFIER_REJECT／HANDOFF_DATA／RECENT_REFS）一律經 `rewrite_feedback(tag, body)` 組成 `<TAG>: <定義句或結構化拒因>`＋`REWRITE_FEEDBACK_SUFFIX`（系統回饋、不是使用者說的話、⛔ 不回覆不複述不確認、重新回答使用者最後一則）；定義句不動。單元 +7（含「迴圈內任何 user 訊息都帶外殼」不變量）＝1921。smoke lb2 一輪 **12/13**、洩漏回合改為正常建議；唯一失分＝回聲確認（模型層）。
- **部署 ②**（`e0fbc6e4`）：線上 conv＋restart 兩會話 6 回合：conv-2／conv-3 不再複誦（conv-3 `recommend`「建議主動打電話…」）；restart-1「還沒說過話」✓；**restart-3 落 `ASK_TARGET_TEXT`**（trace `llm_calls=3 tool_calls=0`：模型三次零查詢轉人、改寫預算 2 用盡 ⇒ 閘門兜底「想處理哪一件事？」而非轉人——設計下限；本地同回合 4 輪 3 過，模型偶發）。第五批候選：判斷題＋最近編號且零查詢 ⇒ 先預查最近編號那筆再回模型（把 `_pre_lookup_trigger` 從「純數字訊息」擴到「有最近編號的零查詢轉人」）。
- 收尾：四個 executor worktree 已移除。**fresh verifier（V1–V4＋F1＋F2）CONFIRMED**：容器內單元 `passed=1921 failed=0`；V3 算術獨立重算 5 案（含不帶 today 的舊等式逐字保住、未來到期日起算＝原到期日）、`today` 三個呼叫點無繞道（兌現閘吃的是雙雜湊綁定的 pending payload）；F1 新舊差分 4 案（兩個誤殺解除、兩個正對照仍擋）、規則檔 56 對實數；F2 用正對照掃描證明 r2 conv-3 複誦在 b4fix 消失、VERIFIER_REJECT body 無文字欄位。發現皆 P3／P4、未阻斷：① `b4_score.py` 看不見複誦型病灶（只擋反問句）——尺要加「回答不得含定義句片語」；② 劇本全在 `grounding_observe` 量，「0 轉人」不等於 enforce 下 0 轉人（pair 類照擋故 F1 證據有效）；③ b4fix 逾期題那一分是模型抽樣差異，不歸因 F2；④ 兩筆 60 s `TOOL_TIMEOUT` 混進功能失分（r2 restart-1、線③ L5-perf#4）；⑤ 卡在未逾期時「原到期日」「起算日」同值兩行（版面）；⑥ F1 放寬後對面非「已＋狀態詞」措辭不擋（主路徑 bills.py 都含肯定形，設計取捨）；⑦ b4fix 受測物是工作樹非 commit（差 3 分鐘、內容同）。未驗：線上實打、enforce 模式端到端、`make audit`、F2 多輪殘餘率。

## 1p. line-bot 第三輪走查（2026-09-09 18:22；報修／帳單線跑在 `7cb9c636`、連續對話線跑在 `e0fbc6e4`；39 屏；scratchpad `linebot_acceptance_20260909c.txt`）

- **修好（他們的表）**：延三天起算日 9/09／新到期日 9/12 且寫入正確；首句 10.2 s（第二輪 60 s）；改描述有出路；物件名單獨一句就認；未結單有 8591；restart「還沒說過話」且分得清引導語；複誦句 11 句 0 出現；轉真人 0（第一輪 8）。
- **未改／新（機制定位，皆查過線上 trace 與 log）**：
  - H10「這戶」不沿用剛解析的物件（帳單線步 7、8 反問哪一戶）：模型 `kind=ask`，最近編號段只在**轉人**時觸發改寫，`ask` 出口沒有閘；根治是「有最近編號的零查詢回合先預查那筆再回模型」（擴 `_pre_lookup_trigger`，資料取代提示）。
  - 步 9「要我把本對話最近出現的編號 756248 當作查詢對象…嗎？」：F2 外殼下模型仍把 RECENT_REFS 提示改寫成確認問句（1/11）；同一根治（給資料不給指令）。
  - **H18「這題要看你的判斷…要我列出來嗎？」灑到電錶度數題**：trace `d51187eb` ＝ 第一版回答被 `SENSITIVE_TOPIC rule#0`（`\d+\s*(元|塊|萬|億|折)`，售前價格守門）擋——電錶答案寫「餘額 4,249 元」即中；改寫後模型轉人 ⇒ `NO_JUDGEMENT_TEXT`。兩個機制問題：① `sensitive_patterns` 不分受眾、不看引用，pm 引資料段的金額被當敏感（帳單題寫 `NT$ 7,500` 只是剛好不中）；② 資料在手仍轉人的出口印「判斷」模板，對資料題文不對題。步 17「那我要不要打電話」的同一句是模型**照抄對話裡步 13 那句固定句**（trace `6fd711ef` kind=ask、無工具、無拒）——固定句進 dialog 會被模仿。
  - H2「要不要打電話」退開場白（trace `3425363a`：零查詢轉人 ×3、改寫預算 2 用盡 ⇒ `ASK_TARGET_TEXT`）：同 H10 根治。
  - **H17 兩次 60 s 逾時，⛔ 不是「單次 OpenAI 呼叫 65–77 s」**（先前口頭說法有誤，對 log 更正）：兩回合都是「請求進來 → 第一次模型呼叫 3 s 回（工具呼叫）→ 之後 57 s 無任何 log → 客戶端 60 s 取消（`POST /mcp 202`）／伺服器 `wait_for` 60 s TOOL_TIMEOUT」；第二次模型請求**從未記到回應**。被取消的回合沒有 trace；緊接的重送 6.4 s／14.3 s 正常（trace `5239355e`、`d507218d` 的 `latency_ms` 是 run_turn 全程 wall-clock）。可排除：替身 API（`ms=0`）、prompt 大小（重送同大小）、429／SDK 重試（2 h 內 0 筆非 200、0 筆 retry）、事件迴圈被同步呼叫卡死（取消通知準時在 +60 s 被記到）。分不出「第二次 OpenAI 請求在途 >57 s」與「送出前卡在某個 await」——伺服器沒有逐次模型呼叫的計時 log。**線上 `OPENAI_TIMEOUT_S` 未設 ⇒ SDK 預設 600 s，一個掛住的請求只能等我們的 60 s**。
  - H8 卡「未結單 2 張」但回答只講 1 張：兩者同一條 `jgb2.query.repairs`（estate_id）資料，模型漏列（`_open_repairs` 與 `query_repairs` 對碼一致）；模型層。
...
## 1v. Plan R 三片＋整理落地（2026-09-10 晚；本地 `df0edb1a`；⛔ 未部署）
...
- R3 fresh verifier **CONFIRMED**（探針 ALL_PASS：`Lifetime` 六值、`KEY_SPECS` 11 鍵、FIFO 20／50／20／5 上限與權威定義相等、`scope_exit()` 只清 `estate_carry`、`end_turn` 三出口行為；反證三條——`HANDOFF_CACHE_MAX` 49 ⇒ 3 紅、`COMPLETED_ACTIONS_MAX` 6 ⇒ 3 紅、`write_last_ask_target` 換 pass ⇒ golden 紅；golden fixture 最後改於 R1b `b62c9327`、R3 未碰，28 條逐位相同）。三條不阻斷、裁決：**A1** P3 全套 `passed=2411`（commit 訊息寫 2410；基準 2390 應為 2391 抄錄差 1，非漏測）→ 文件此處更正、不再追；**A2** P4 `recent_refs` 閘判定式由 `select_scope is None` 改為 `scope_estate_id is None`，只在 `select_scope` 為無 `estate_id` 的 dict 時分歧、全歷史指派形狀查無此形 → DEFER，併入「golden 補情境」候選（加 `select_scope={"type":"bills"}` 一條即可分辨）；**A3** P4 `begin_turn` 零呼叫者 no-op、`runtime.py` 仍 import 未用的 `_append_dialog`／`_trim_handoff_cache` → DEFER 併 R3b（facade `outline` 走 AgentSession 時一起接線／刪）。未驗：`make audit`（dev 容器映像過期、不變量 3 結構性紅）、真模型 smoke 對照（純結構收斂，golden 逐位相同故不補）。
