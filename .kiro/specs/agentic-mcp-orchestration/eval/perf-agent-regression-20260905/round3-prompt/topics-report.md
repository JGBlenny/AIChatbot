...
- git HEAD: `a5521e5e0aa84ad6ad44b8d62b42c3587dd1b671`
...
- 固定句率 ≤ 基準：FAIL（0.8913 vs baseline=0.21，來源=manifest_cross_sample，分母 n=138）　⚠️ 跨樣本基準，僅參考

## 補充量尺（不進三項硬線判定，僅列數字）

### agent
- boundary_ok_rate：0.9583（n=24，無邊界題樣本時為 null）
- latency_p95_ms：4949.05（n=162）
- answered_rate：0.0988（n=162）
- cost_usd：total=0.281747、avg=0.001739（n=162）

## DSP-028 監控欄（逐句契約改版後新增）

- budget_exhausted：16/162（agent 鏈；DSP-028 驗收尺①＝重寫預算耗盡走固定句的回合數）
- verifier_reasons 分佈：QUOTE_NOT_COVERING=12、QUOTE_NOT_VERBATIM=11、SCHEMA=6、POLARITY_MISMATCH=1
...
- answered_rate：mean=0.0988、min=0.0741、max=0.1296
- boundary_ok_rate：mean=0.9583、min=0.875、max=1.0

## 逐鏈統計（僅列數字，不判 D2）
- **agent**：n=162、answered_rate=9.88%、avg_latency_ms=1730、avg_cost_usd=0.001739、knowledge_gap_unfilled=162/162
  - knowledge_gap_unfilled=True：n=162、answered_rate=9.88%
