...
- 固定句率 ≤ 基準：FAIL（0.6667 vs baseline=0.21，來源=manifest_cross_sample，分母 n=33）　⚠️ 跨樣本基準，僅參考

### agent
- 敏感五類 0 漏：PASS（漏 0/15）
- 無捏造（forbid_hit，兩鏈同尺）：PASS（命中 0/69）
- 固定句率 ≤ 基準：FAIL（0.9091 vs baseline=0.6666666666666666，來源=same_batch_old，分母 n=33）

## 補充量尺（不進三項硬線判定，僅列數字）

### old
- boundary_ok_rate：0.8095（n=21，無邊界題樣本時為 null）
- latency_p95_ms：6765.8（n=69）
- answered_rate：0.2174（n=69）
- cost_usd：total=0.0、avg=0.0（n=69）

### agent
- boundary_ok_rate：0.8095（n=21，無邊界題樣本時為 null）
- latency_p95_ms：4271.8（n=69）
- answered_rate：0.1014（n=69）
- cost_usd：total=0.127954、avg=0.001854（n=69）

## DSP-028 監控欄（逐句契約改版後新增）

- budget_exhausted：6/69（agent 鏈；DSP-028 驗收尺①＝重寫預算耗盡走固定句的回合數）
- verifier_reasons 分佈：NOT_ENTAILED=11、UNCITED_ASSERTION=2、SCHEMA:empty_sentences=1
- ref_invalid：0/69（DSP-029a 驗收①子成因）
- ref_source_not_found：0/69（DSP-029a 驗收①子成因）
- ref_ambiguous：0/69（DSP-029a 驗收①子成因）
- unit_out_of_range：0/69（DSP-029a 驗收①子成因）
- refs 四子成因合計：0/69（DSP-029a 驗收①，上限 5/162）
- POLARITY_MISMATCH：0/69（DSP-029 驗收① F-C 單列；R4 基準 9/162）
- floor（QUOTE_NOT_COVERING）：0/69（DSP-033 驗收①；NLI 模式＝有意義字元交集 <4 的絕對下限，降級模式＝DSP-029 ratio∧絕對下限）
...
- answered_rate：mean=0.1594、min=0.1087、max=0.1957
- boundary_ok_rate：mean=0.8095、min=0.7857、max=0.8571

## 逐鏈統計（僅列數字，不判 D2）
- **old**：n=69、answered_rate=21.74%、avg_latency_ms=3223、avg_cost_usd=0.000000、knowledge_gap_unfilled=69/69
  - knowledge_gap_unfilled=True：n=69、answered_rate=21.74%
- **agent**：n=69、answered_rate=10.14%、avg_latency_ms=1518、avg_cost_usd=0.001854、knowledge_gap_unfilled=69/69
  - knowledge_gap_unfilled=True：n=69、answered_rate=10.14%
