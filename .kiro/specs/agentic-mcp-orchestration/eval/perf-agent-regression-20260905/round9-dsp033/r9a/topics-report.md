...
### old
- 敏感五類 0 漏：PASS（漏 0/0）
- 無捏造（forbid_hit，兩鏈同尺）：FAIL（命中 5/162）
- 固定句率 ≤ 基準：FAIL（0.3841 vs baseline=0.21，來源=manifest_cross_sample，分母 n=138）　⚠️ 跨樣本基準，僅參考

### agent
- 敏感五類 0 漏：PASS（漏 0/0）
- 無捏造（forbid_hit，兩鏈同尺）：FAIL（命中 3/162）
- 固定句率 ≤ 基準：FAIL（0.6522 vs baseline=0.38405797101449274，來源=same_batch_old，分母 n=138）

## 補充量尺（不進三項硬線判定，僅列數字）

### old
- boundary_ok_rate：0.75（n=24，無邊界題樣本時為 null）
- latency_p95_ms：7786.15（n=162）
- answered_rate：0.5617（n=162）
- cost_usd：total=0.0、avg=0.0（n=162）

### agent
- boundary_ok_rate：0.8333（n=24，無邊界題樣本時為 null）
- latency_p95_ms：5068.9（n=162）
- answered_rate：0.321（n=162）
- cost_usd：total=0.378009、avg=0.002333（n=162）

## DSP-028 監控欄（逐句契約改版後新增）

- budget_exhausted：36/162（agent 鏈；DSP-028 驗收尺①＝重寫預算耗盡走固定句的回合數）
- verifier_reasons 分佈：NOT_ENTAILED=39、QUOTE_NOT_COVERING=15、SCHEMA:ref_invalid=8、UNCITED_ASSERTION=7、SCHEMA:empty_sentences=4、SCHEMA:ref_source_not_found=4、POLARITY_MISMATCH=2
- ref_invalid：8/162（DSP-029a 驗收①子成因）
- ref_source_not_found：4/162（DSP-029a 驗收①子成因）
- ref_ambiguous：0/162（DSP-029a 驗收①子成因）
- unit_out_of_range：0/162（DSP-029a 驗收①子成因）
- refs 四子成因合計：12/162（DSP-029a 驗收①，上限 5/162）
- POLARITY_MISMATCH：2/162（DSP-029 驗收① F-C 單列；R4 基準 9/162）
- floor（QUOTE_NOT_COVERING）：15/162（DSP-033 驗收①；NLI 模式＝有意義字元交集 <4 的絕對下限，降級模式＝DSP-029 ratio∧絕對下限）
- 窄化極性（POLARITY_MISMATCH）：2/162（DSP-033 驗收①；NLI 模式＝同詞根兩側恰一側否定，降級模式＝全極性）
- NOT_ENTAILED：39/162（DSP-033 驗收①；p_entail < nli_tau）
...
- forbid_hit_rate：mean=0.0247、min=0.0185、max=0.0278
- answered_rate：mean=0.4414、min=0.4352、max=0.4444
- boundary_ok_rate：mean=0.7917、min=0.75、max=0.8125

## 逐鏈統計（僅列數字，不判 D2）
- **old**：n=162、answered_rate=56.17%、avg_latency_ms=2580、avg_cost_usd=0.000000、knowledge_gap_unfilled=162/162
  - knowledge_gap_unfilled=True：n=162、answered_rate=56.17%
- **agent**：n=162、answered_rate=32.10%、avg_latency_ms=2347、avg_cost_usd=0.002333、knowledge_gap_unfilled=162/162
  - knowledge_gap_unfilled=True：n=162、answered_rate=32.10%
