...
- git HEAD: `7754fb0d657ef5a34a92d65f25f664b76a93dc65`
...
- 固定句率 ≤ 基準：FAIL（0.7971 vs baseline=0.21，來源=manifest_cross_sample，分母 n=138）　⚠️ 跨樣本基準，僅參考

## 補充量尺（不進三項硬線判定，僅列數字）

### agent
- boundary_ok_rate：0.9167（n=24，無邊界題樣本時為 null）
- latency_p95_ms：5986.05（n=162）
- answered_rate：0.1852（n=162）
- cost_usd：total=0.323366、avg=0.001996（n=162）

## DSP-028 監控欄（逐句契約改版後新增）

- budget_exhausted：53/162（agent 鏈；DSP-028 驗收尺①＝重寫預算耗盡走固定句的回合數）
- verifier_reasons 分佈：QUOTE_NOT_VERBATIM=51、SCHEMA=28、UNCITED_ASSERTION=16、QUOTE_NOT_COVERING=12、POLARITY_MISMATCH=9
...
- answered_rate：mean=0.1852、min=0.1296、max=0.2222
- boundary_ok_rate：mean=0.9167、min=0.875、max=1.0

## 逐鏈統計（僅列數字，不判 D2）
- **agent**：n=162、answered_rate=18.52%、avg_latency_ms=2704、avg_cost_usd=0.001996、knowledge_gap_unfilled=162/162
  - knowledge_gap_unfilled=True：n=162、answered_rate=18.52%
