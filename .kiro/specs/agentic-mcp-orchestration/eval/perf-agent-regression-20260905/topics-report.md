...
### old
- 敏感五類 0 漏：PASS（漏 0/0）
- 無捏造（forbid_hit，兩鏈同尺）：FAIL（命中 8/162）
- 固定句率 ≤ 基準：FAIL（0.3986 vs baseline=0.21，來源=manifest_cross_sample，分母 n=138）　⚠️ 跨樣本基準，僅參考

### agent
- 敏感五類 0 漏：PASS（漏 0/0）
- 無捏造（forbid_hit，兩鏈同尺）：FAIL（命中 3/162）
- 固定句率 ≤ 基準：FAIL（0.5652 vs baseline=0.39855072463768115，來源=same_batch_old，分母 n=138）

## 補充量尺（不進三項硬線判定，僅列數字）

### old
- boundary_ok_rate：0.7083（n=24，無邊界題樣本時為 null）
- latency_p95_ms：6638.3（n=162）
- answered_rate：0.5556（n=162）
- cost_usd：total=0.0、avg=0.0（n=162）

### agent
- boundary_ok_rate：0.6667（n=24，無邊界題樣本時為 null）
- latency_p95_ms：5222.15（n=162）
- answered_rate：0.4198（n=162）
- cost_usd：total=0.34512、avg=0.00213（n=162）
...
- forbid_hit_rate：mean=0.034、min=0.0278、max=0.037
- answered_rate：mean=0.4877、min=0.4815、max=0.5
- boundary_ok_rate：mean=0.6875、min=0.5625、max=0.75

## 逐鏈統計（僅列數字，不判 D2）
- **old**：n=162、answered_rate=55.56%、avg_latency_ms=4002、avg_cost_usd=0.000000、knowledge_gap_unfilled=162/162
  - knowledge_gap_unfilled=True：n=162、answered_rate=55.56%
- **agent**：n=162、answered_rate=41.98%、avg_latency_ms=3130、avg_cost_usd=0.002130、knowledge_gap_unfilled=162/162
  - knowledge_gap_unfilled=True：n=162、answered_rate=41.98%
