...
| P1 | MCP 串流契約未定 | FIX：定為一次性回傳；語音（LINE 語音訊息）經 `agent.turn` `audio_urls` 轉錄後進同一回合、回應第六鍵 `transcript`（DSP-041；⛔ 不再指向 roadmap `voice-turn-budget`）
