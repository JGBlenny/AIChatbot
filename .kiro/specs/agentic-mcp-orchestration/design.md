...
> 1.5 變更（2026-09-08，demo 上線大改版；細節見 `docs/architecture/AGENTIC_MCP_ARCHITECTURE.md` 與 `.kiro/specs/knowledge-outline-and-intent-architecture/HANDOFF-20260909.md`）：`agent.turn` 對 property_manager 亦開 M1（DSP-037）；寫入工具 `jgb2.action.{bill_due_extend,repair_create}`＋`confirm.request` 確認鏈（DSP-038：旗標×stage 可見、卡文字程式產出且雜湊綁卡、`pending_id`、待確認存 session 狀態）；Verifier 引用類只記錄（DSP-040，demo 過渡旗）；清單點選 `select:<type>:<id>` 與會話過期 `session_expired` 第六鍵（DSP-042）；`jgb2.query.repairs` 選填 `estate_id`；select 後別戶邊界＝`estate_id` 程式判定（L15）；照片進場 `image_urls`（W8 (2)：服務端自抓 relay 簽章網址、≤10 張、決定性辨識驗證、`run_turn(image=ImageTurnInput)`）；語音 `audio_urls`／`transcript`（W7）**未落地**。
...
| `jgb2.query.<domain>` | `{face: <domain 的封閉 enum>, ref?: str, keyword?: str}`；**`repairs` 另有選填 `estate_id: str`**（DSP-042／W8 (3)：`_jgb2_spec` `extra_properties` 只對 repairs，其餘域逐位元不變；handler 透傳 `get_repairs(estate_id=…)`） | `{facts: str, candidates?: [...], candidate_cap, skip_refine: bool}` | domain→(API 方法, 註冊表, secondary) 見下方**域映射表**（⛔ 不用 `get_<domain>` 推導）；`face` 由模型在封閉 enum 中選（取代 categories 提名）；`ref`／`keyword` 只能在 session 已確立的 slot 範圍內縮小；無 slot 時 keyword 查詢回傳 ≤ `CANDIDATE_CAP`（預設 5）候選，候選數 ≤ cap ⇒ `skip_refine=true` 全列供 `confirm`，否則要求縮小（承接 R3.3）；圈定實際邊界逐域見映射表（DSP-011）；標籤讀回應 `mapping`，⛔ 不用 `bills.STATUS_LABELS`（缺口 7） |
...
**`agent.turn` 工具（1.4）**：`agent.turn(message, image_urls?) -> {answer, kind, handoff, quick_replies, trace_id, session_expired, outcome, transcript?}`（五鍵固定；第六鍵 `session_expired: bool` DSP-042、第七鍵 `outcome{state, expects, action, ref}` DSP-043（八態封閉、程式設）、第八鍵 `transcript` DSP-041 未落地），scope=read、`stage={prospect: M1, property_manager: M1}`（DSP-037）、**`facade_only=True`**（不進模型工具清單；Runtime 對 registry 的 `call()` 一律 `for_model=True` ⇒ 模型捏造此名亦 `NO_MATCH`，無自呼遞迴；不進影子視圖 ⇒ 影子不寫 session）；**註冊受 `AGENT_TURN_ENABLED` 控（回切開關）**；外部 MCP client 本身即模型，對它 `facade_only` 為單層設計意圖（每呼叫計額）；`agent.turn` ⛔ 不受 `AGENT_AUDIENCES` 左右（只管 REST 入口）；**狀態命名空間**：MCP 進來的回合以 `mcp:{api_key_id}:{vendor_id}:{session_id}` 為 `form_sessions` 鍵（REST 不變），⛔ 不得以純 session_id 讀寫（前置 security review P1）；schema 收 `message`（可空）、選填 `image_urls`（DSP-042／W8 (2)，**已落地**：≤10 張 relay 簽章網址、服務端自抓）與選填 `audio_urls`（DSP-041／W7，**尚未落地**）；`message` 與媒體不得皆空；獨立逾時 `AGENT_TURN_TIMEOUT_S`（>Budget.deadline_s）；LLM token 進 `add_llm_usage`；每小時上限 `AGENT_TURN_CAP`；~~loopback 免計量標頭~~ 已於 1.4.6 撤回（可偽造、違反不變量 31）——M1–M3 Runtime 一律程式內呼叫 registry；一次 `agent.turn` 對應一列 `usage_events`，內含的多次 LLM 呼叫以事件層 token 欄彙總；門面以解析出的 Identity 呼叫同一個 `AgentRuntime.run_turn`（Verifier、固定句、預算、計量全同），⛔ 不另寫回合邏輯；一次性回傳（MCP 工具結果不逐字串流，語音（LINE 語音訊息）經 `agent.turn` `audio_urls` 轉錄後進同一回合、回應第七鍵 `transcript`（DSP-041；⛔ 不再指向 roadmap `voice-turn-budget`；**W7 尚未落地，現行 `AgentTurnOutput` 為七鍵＝五鍵＋`session_expired`＋`outcome`**）；**清單點選**（DSP-042）：`message` 整句 `fullmatch` `select:<type>:<id>`（`type ∈ bill|contract|repair`）⇒ Runtime 在模型前攔截、以 `_SELECT_DEFAULT_FACE` 封閉表走 `jgb2.query.<type 對應域>`（`registry.call`，授權由 JGB API 依 `role_id` 收口），facts 出口再過 `_verify_routes`，寫槽位 `<type>_ref`、作廢尚無 receipt 的 `pending_confirm`，dialog 只寫程式摘要，trace 只記 `select_type`／`has_ref`；不存在與不在範圍同一句「查無此筆」；**會話過期**（DSP-042）：`agent_state["last_turn_at"]` 超過 30 分鐘 ⇒ 先 `_close` 舊列再 `_start`，該回合 `session_expired=true`。**身分與 session 契約**：`session_id` 由呼叫端產生、跨回合穩定；對話歷史與 slots 由服務端存 `form_sessions.collected_data`（與 REST 同源）；`role_id=null`＋`target_user=tenant` 為合法租客組合（可見性＝tenant 池，jgb2.query 因缺 role_id 雙證而 `NO_MATCH`）；`/mcp` 僅 server-to-server。
...
~~class SentenceCite(BaseModel): sent: int; kind: ...; cite: list[int]~~   # 1.4.10（DSP-028）刪，改 Sentence（見下）

class AgentOutput(BaseModel):   # response_format json_schema strict
    kind: Literal["answer","ask","recommend","handoff"]; citations: list[Citation]  # 1.4.10：answer 改導出，不在模型輸出
    sentences: list[Sentence]  # 1.4.10（DSP-028）：{text, kind, cite[]}；~~answer: str~~ 改為導出屬性 answer = "".join(s.text)；~~sentence_map~~ 刪
    fact_class: FactClass; handoff_reason: Optional[str]

class Sentence(BaseModel):  # 1.4.10
    text: str                                   # 含句尾標點；一筆一句（多句尾 ⇒ Verifier 切片段各自驗）
    kind: Literal["fact", "question", "greeting", "routing"]
    cite: list[int] = []                        # citations 索引；fact 必填
...
② **白名單句型**：**1.4.10（DSP-028，已落地 2026-09-05）**：模型輸出 `sentences[{text, kind, cite[]}]`、`answer` 由程式拼接（⛔ 不再要模型數句子——4.3c 實測 47% 回合因句數對不上被拒到固定句）；**1.4.11（DSP-029，提案 v4）**：`Citation{tool_call_id, source, unit}`，引文由 Runtime 依 `(tool_call_id, source, unit)` 從 `Provenance.text` 以 `provenance_units` 切出的第 unit 句解析，另以 `resolved` 傳 Verifier（⛔ 不進 AgentOutput）；unit 只由 provenance 逐筆產生；標記 `[{nonce}:{source}§{i}]` 由 `wrap_provenance_data` 於 sanitize 後貼（大綱在 `PromptAssembler.build` 以 `outline.sections` 生成、`_build_doc` 產物未標記；工具在 runtime 組 tool 訊息時貼；簽名與 R11.5 不動）；③ 覆蓋改片段側相對覆蓋率 0.5＋絕對下限 4；`answer_nfkc` 含標記樣式 ⇒ SCHEMA(`marker_in_answer`)；`VerifierVerdict.schema_cause` 封閉 Literal；`kb.search` provenance `citable=False`；R4 三捏造句進 `known_open.json`（仍放行、單列計數，由 DSP-030 處理）。逐筆檢查：`kind≠handoff` 且 `sentences` 空 ⇒ SCHEMA；`text` 非空、`cite` 索引在範圍；一筆內多句尾則切片段，**每片段各自跑型別複核與步③～④、比對文字用片段本文**（繼承的是標籤不是驗證結果——r2 安全審第 10 條「疑問句夾斷言」由此擋）；`VerifierVerdict.sent`＝筆索引；~~`sentence_map` 必須覆蓋 `answer` 全文（NFKC 後各句拼接 == answer，否則 `SCHEMA`）~~；每句必標 kind；`question`／`greeting`／`routing` 三型以程式端封閉判定複核——**「純」條件**：以逗號／頓號／分號切子句，任一子句命中 R6.2 封閉詞集（可以／支援／不支援／需要／會／不會／無法…，版本化於 `VerifierRules.assertion_terms`）⇒ 整句降級為 `fact`；`question` 另需問號結尾、`greeting` 需全句在問候詞表、`routing` 需只含白名單 route。**`fact` 一律需 ≥1 cite**（預設反轉：白名單句型才免 cite，黑名單詞集只用來撤銷豁免），缺 ⇒ `UNCITED_ASSERTION`。例：「支援批次匯入合約，請問您有幾間？」⇒ 子句一命中「支援」⇒ fact ⇒ 需 cite（承接 R6.2／R6.7）。
...
sequenceDiagram
    ...
    alt 多筆候選
        ...
        M-->>R: AgentOutput kind=answer{sentences[{text,kind,cite}], citations, fact_class}（1.4.10）
    end
...
9. **D1 已裁（業主 2026-09-05，DSP-023）：統一 gpt-4o-mini**，與舊鏈同模型、⛔ 不為 agent 另開 gpt-4o；`AGENT_MODEL` 僅供本機對照實驗。原假設：先用 OpenAI function calling（provider 抽象保留）；**D2 已裁（業主 2026-09-05，DSP-025，數字見 manifest `acceptance_d2`）**；D3 幫助中心、D4 寫入首批、D5 影子月上限 **仍待業主**——D3 未裁前 `help_center_pages.citable` 全 false；D2 未裁前 `agent_eval` 只出對照不判 PASS。阻擋：D2→M2、D3→M1 的 help 引用、D4→M4、D5→M1 影子開啟。
...
## 部署考量
新 env：`AGENT_STAGE`、`AGENT_TURN_ENABLED`（預設 false）、`AGENT_AUDIENCES`、`AGENT_SHADOW_AUDIENCES`、`AGENT_SHADOW_MONTHLY_USD_CAP`、`AGENT_BUDGET_TOOL_CALLS/REWRITES/DEADLINE_S`、`AGENT_OUTLINE_TOKEN_LIMIT_PROSPECT`、`MCP_ALLOWED_ORIGINS`、`RATE_PER_MIN`、`KB_GET_SESSION_CAP`；依賴 `mcp==2.1.1`、`tiktoken==0.14.0`；migration：三張新表。步驟同現行 runbook；⛔ 線上由業主執行。監控：每回合成本 > ×3、影子月上限、Verifier 拒率分佈、`tool_unavailable` 率、p95。
...
| P1 | MCP 串流契約未定 | FIX：定為一次性回傳；語音（LINE 語音訊息）經 `agent.turn` `audio_urls` 轉錄後進同一回合、回應第七鍵 `transcript`（DSP-042 後）（DSP-041；⛔ 不再指向 roadmap `voice-turn-budget`）
