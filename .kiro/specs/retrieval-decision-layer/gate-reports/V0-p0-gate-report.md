...
> # ⛔ 已退回（2026-08-19），本報告不得作為 V0 已過的依據
> 反證盤查發現**量測工具本身有四項 P2 缺陷**（量尺對知識接地退化全盲、快照覆蓋僅 54.8%
> 且面向續輪 0%、不變量 8 八種寫法只擋一種、語料閘門自簽自證）。本報告內所有量化數字
> （不一致 1/107、基線 5/107、E-5 上限 ≤3/104）均為缺陷尺讀數，**一律作廢**。
> 仍然有效者僅：任務 1.3 的門檻收斂與六 case 嚴格等價（與尺無關）。
> 須以修復後的尺重出本報告。見 `decisions/DECISIONS.md`、`tasks.md` 驗收狀態表。
