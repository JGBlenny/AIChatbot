...
## ⛔ 2026-09-01 更正：本檔原本的影響評估**錯了**

```text
原宣稱  「生產檢索 top-20 召回率 31%、top-1 不同 46%、8 題答不出來」
實際    ⛔ 那是**我自寫診斷 SQL** 的行為，不是生產路徑
        生產用 `ORDER BY (1 - (embedding <=> v)) DESC`（算式＋DESC）⇒ **用不上向量索引**
        一律精確掃描；我的 SQL 用正規寫法 `ORDER BY embedding <=> v` 才會走索引
實證    35 題經正式入口重跑，**濾後結果修前修後逐筆相同**（top-1 改變 0/35）
```

⇒ 正確定性：**潛在地雷，非正在流血的傷口**。
危險在於 `ORDER BY (1 - dist) DESC` 是低效但正確的寫法——
任何人為效能把它改成正規寫法（那是**標準做法**），召回率就會當場崩塌且不報錯。
改成 HNSW 之後那個優化才安全。

⚠️ 另一個更重要的更正：**本檔所有量測都跑在 reranker 已停用的期間**
（見 `scripts/BACKLOG.md` 的 reranker 條目）。rerank 佔最終分數 90%，
⇒ 這些數字的絕對值 ⛔ 不得引用，只有「修前後相同」這個相對結論成立。
