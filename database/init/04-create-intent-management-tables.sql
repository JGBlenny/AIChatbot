...
-- 2. business_scope_config 表（業務範圍配置）
...
-- 4. 為 knowledge_base 表添加外鍵約束
-- 注意：欄位已在 02 腳本中定義，此處只添加外鍵約束
ALTER TABLE knowledge_base
ADD CONSTRAINT fk_knowledge_intent FOREIGN KEY (intent_id) REFERENCES intents(id);
...
-- 5. 插入測試意圖數據
INSERT INTO intents (name, type, description, keywords, is_enabled, priority)
VALUES
    ('帳務查詢', 'knowledge', '租金繳費、帳單查詢、逾期費用等財務相關問題', ARRAY['繳費', '租金', '帳單', '逾期', '手續費'], true, 10),
    ('退租流程', 'knowledge', '退租申請、押金退還、合約終止等退租相關問題', ARRAY['退租', '押金', '解約', '提前解約'], true, 10),
    ('報修問題', 'knowledge', '設備故障、維修申請、緊急報修等維護相關問題', ARRAY['報修', '維修', '故障', '壞掉'], true, 10),
    ('租約查詢', 'data_query', '租期查詢、合約內容、租約條款等查詢', ARRAY['租約', '合約', '租期', '到期'], true, 8),
    ('物件資訊', 'knowledge', '房屋設備、使用說明、社區規範等物件相關資訊', ARRAY['設備', '使用', '規範', '禁止'], true, 5)
