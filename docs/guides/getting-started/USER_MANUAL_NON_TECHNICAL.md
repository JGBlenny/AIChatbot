...
```
使用者提問 → 檢查緩存 → 搜尋 SOP/知識庫 → 優化答案 → 返回結果
```
...
系統使用**向量檢索 + Reranker 重排序**來理解使用者問題，透過語義相似度找到最相關的 SOP 或知識庫內容。系統能處理各類問題，包括但不限於：
...
當系統找不到相關內容時，會記錄問題並返回兜底答案，引導用戶聯繫客服。
