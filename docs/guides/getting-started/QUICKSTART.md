...
> ⚠️ **2026-09-11 舊鏈退役（本輪文件盤查連帶發現，非原七條指定範圍）**：下方
> 「發送問題測試」一段的 `POST /api/v1/message` 已刪除（2026-09-11 砍除舊 REST
> 對話鏈），`llm_answer_optimizer`／Phase 3 相關描述也已隨之退役。對話能力
> 現在只經 `/mcp` 門面（server-to-server），契約見 `docs/api/mcp-facade.md`；
> 本指南其餘啟動/健康檢查步驟不受影響。
...
- [pgvector 設定說明（已歸檔，歷史快照）](../../archive/2026-09/PGVECTOR_SETUP.md)——新環境建置走 dump 還原，見 docs/deployment-runbook.md 附錄 A
- 知識庫管理說明
