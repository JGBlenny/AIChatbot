...
- ⚠️ **匯出功能**：有限（僅支援文件轉換模組，缺少知識庫批次匯出）
- ❌ **備份還原**：無
- ❌ **資料遷移**：無

### 規劃重點
1. **新增完整匯出功能**（優先級：高）
2. **增強匯入功能**（PDF 支援、平行處理）
3. **備份與還原機制**（資料安全）
4. **跨環境遷移工具**（開發/生產環境同步）
...
#### ⚠️ 現狀
僅在 **文件轉換模組** 中提供有限的 CSV 匯出：
- 端點：`POST /api/v1/document-converter/{job_id}/export-csv`
- 格式：CSV
- 欄位：`question_summary`, `content`, `keywords`, `intent_id`

#### ❌ 缺失
1. **無法從正式知識庫（knowledge_base）匯出**
2. **無批次匯出**
3. **無格式選擇**（僅 CSV）
4. **無篩選條件**（無法按意圖、分類、日期等篩選）
5. **無匯出歷史追蹤**
