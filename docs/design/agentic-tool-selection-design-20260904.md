# 決策搬進模型：Agent 自選工具架構設計（提案，2026-09-04）
...
2. `kind=answer` 時，每個含產品事實的句子（含「可以／支援／不支援／需要／會」等斷言）必須至少一個 cite；無 cite 的句子只允許問候、反問、導流三型（封閉集合）。
3. 敏感五類（客戶案例、價格數字、SLA、法遵、資安）：answer 內出現數字或機構名而 citations 無同字串 ⇒ 拒。
...
| `presales_gate`、R2.7–R2.13 閘門、`_top1_relevance_gate`、`decide_arbitration` | **退休**（功能由工具邊界＋輸出契約承接） |
