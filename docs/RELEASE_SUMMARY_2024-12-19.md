...
### 1. `2acd9e7` - feat: 新增意圖 embedding 生成腳本與完整部署流程 ⭐ **LATEST**
**類型**: 功能增強 + 文檔
**影響範圍**: 部署流程 + 語義匹配

**新增文件**:
- `scripts/generate_intent_embeddings.py` (250 行)

**變更文件**:
- `docs/DEPLOYMENT_STEPS_2024-12-19.md` (+90 行)

**核心功能**:
- 🎯 意圖 Embedding 生成器
  - 結合意圖名稱和描述生成向量
  - 批量處理所有意圖
  - 支持增量更新（只處理缺少 embedding 的）
  - 支持完整更新（--all 覆蓋現有）
  - 實時進度追蹤（每 10 個顯示）
  - 成功率統計與錯誤處理
...
**部署文檔更新**:
- 步驟 3 拆分為 3.1 和 3.2
  - 3.1: 知識庫 Embeddings（Keywords）
  - 3.2: 意圖 Embeddings（語義匹配）⭐ **NEW**
- 新增資料庫遷移步驟
- 新增驗證步驟
- 新增故障排除（問題 1.5）
- 更新檢查清單（+3 項）
...
**執行時間**: 1-3 分鐘（20-50 個意圖）

---

### 2. `b7b4da1` - feat: 語義意圖匹配與多意圖知識支援
**類型**: 功能增強 + 文檔
**影響範圍**: 語義匹配 + 資料庫

**新增文件**:
- `database/migrations/add_intent_embedding.sql` - 資料庫遷移腳本
- `docs/DEDUPLICATION_SUMMARY.md` (204 行) - 去重機制說明
- `docs/MULTI_INTENT_SCORING.md` (293 行) - 評分機制說明

**變更文件**:
- `.gitignore` - 排除測試文件

**核心功能**:
- 資料庫遷移：添加 `intents.embedding` 欄位（1536 維）
- 為未來的 IVFFlat 索引做準備
- 多意圖知識去重機制
- 智能評分邏輯（1.0x ~ 1.3x boost）
...
### 3. `ef8ad19` - docs: 新增 2024-12-19 發布總結報告
**類型**: 文檔
**影響範圍**: 發布管理
...
### 4. `c5c3450` - docs: 新增 2024-12-19 完整部署步驟文檔
**類型**: 文檔
**影響範圍**: 部署流程
...
### 6. `f05f5fb` - feat: 後端服務功能增強與語義匹配改進
**類型**: 功能增強
**影響範圍**: 後端服務
...
### 7. `b6b0bd2` - chore: 資料庫 schema 與 Docker 配置優化
**類型**: 配置優化
**影響範圍**: 基礎設施
...
### 9. `9a5d35c` - feat: 完整實現 keywords 融入 embedding 與 UI 優化
**類型**: 核心功能
**影響範圍**: 全棧
...
### Embedding 更新
**知識庫 Embeddings** (Keywords):
- ✅ 處理筆數: 1240/1240
- ✅ 成功率: 100%
- ✅ 執行時間: ~20 分鐘
- ✅ 日誌完整: `/tmp/embedding_update.log`

**意圖 Embeddings** (語義匹配):⭐ **NEW**
- ✅ 資料庫遷移: intents.embedding 欄位已添加
- ✅ 生成腳本: generate_intent_embeddings.py 已就緒
- ✅ 預期處理: 20-50 個意圖
- ✅ 預期時間: 1-3 分鐘
