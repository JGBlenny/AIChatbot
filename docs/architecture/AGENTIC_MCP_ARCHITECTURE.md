...
LINE 業務（房東管家）的一句話進 `/mcp` 的 `agent.turn`，Runtime 先用**程式**處理三種機器值（清單點選、確認按鈕、過期），其餘交模型在封閉工具清單裡查 JGB 替身資料；要寫入一律先出確認卡、按了才兌現；答案在觀察模式的 Verifier 下只記不擋，機敏面仍由程式與規則守。
...
| Verifier | `services/agent/verifier.py`（`OutputVerifier`、`_verify_routes`）、`app.py`（`_wrap_verifier_observe_only`） | 引用類拒因只記錄（DSP-040 觀察模式）；URL／電話導流仍擋 | 觀察旗只在 `USE_MOCK_JGB_API=true` 允許 |
...
| 健檢 | `services/agent/health.py` | `write_tools_enabled`、`use_mock_jgb_api`、`verifier_observe_only`、`image_recognition{…}` | — |
...
| `AGENT_VERIFIER_OBSERVE_ONLY` | true | 過渡旗（DSP-040）；只在 mock 允許；W6-b3 後改 `AGENT_VERIFIER_MODE` |
...
⛔ 不設 `AGENT_BUDGET_REWRITES=0`（探針組態）、不設 `AGENT_ATTEMPT_LOG_PATH`（開發量測）。
...
- 過渡旗 `AGENT_VERIFIER_OBSERVE_ONLY` 連機敏題也不擋，敏感題由模型自行轉人；正式參數待 W6-b3。
