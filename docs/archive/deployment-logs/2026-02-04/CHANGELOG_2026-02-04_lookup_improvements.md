# Lookup Table 系統優化 - 2026-02-04

## 概述

優化電費寄送區間查詢系統，解決地址匹配問題，提升用戶體驗。

## 主要更新

### 1. 地址資料清理

**問題：** 資料庫中的地址包含備註後綴（如 "(電費需做切算)"、"-租客自繳"），導致精確匹配失敗。

**解決方案：**
```sql
-- 移除括號內容
UPDATE lookup_tables
SET lookup_key = REGEXP_REPLACE(lookup_key, '\([^)]*\)', '', 'g')
WHERE category = 'billing_interval' AND vendor_id = 1 AND lookup_key ~ '\(';

-- 移除 "-租客自繳" 後綴
UPDATE lookup_tables
SET lookup_key = REGEXP_REPLACE(lookup_key, '-租客自繳', '')
WHERE category = 'billing_interval' AND vendor_id = 1 AND lookup_key LIKE '%-租客自繳';

-- 移除 "樓下" 後綴
UPDATE lookup_tables
SET lookup_key = REGEXP_REPLACE(lookup_key, '樓下', '')
WHERE category = 'billing_interval' AND vendor_id = 1 AND lookup_key LIKE '%樓下';
```

**成果：**
- 清理 19 筆記錄
- 247 筆資料完整保留（單月 29 筆、雙月 191 筆、自繳 27 筆）

---

### 2. 模糊匹配多選項檢測

**問題：** 用戶輸入不完整地址時（如 "新北市三重區重陽路3段158號樓"），系統找到多個相似度相同的樓層（一、二、三、四樓），但只返回第一個，可能返回錯誤結果。

**解決方案：**

#### 2.1 Lookup API 改進
**檔案：** `/Users/lenny/jgb/AIChatbot/rag-orchestrator/routers/lookup.py`

```python
# 檢查是否有多個相似度接近的匹配（差距小於 2%）
ambiguous_threshold = 0.02
similar_matches = [
    m for m in match_scores
    if abs(m['score'] - best_score) < ambiguous_threshold
]

# 如果有多個相似度接近的匹配，要求提供完整地址
if len(similar_matches) > 1:
    return {
        "success": False,
        "error": "ambiguous_match",
        "suggestions": [
            {
                "key": m['key'],
                "score": round(m['score'], 2),
                "value": matched_row['lookup_value']
            }
            for m in similar_matches
        ],
        "message": "您輸入的地址不夠完整，找到多個可能的匹配。請提供完整的地址（包含樓層等詳細資訊）。"
    }
```

#### 2.2 錯誤訊息格式化
**檔案：** `/Users/lenny/jgb/AIChatbot/rag-orchestrator/services/universal_api_handler.py`

```python
# 處理模糊匹配（地址不完整）
if error_type == 'ambiguous_match' and suggestions:
    suggestion_text = "\n\n**找到以下可能的地址，請選擇或提供完整地址：**\n"
    for i, sug in enumerate(suggestions, 1):
        value_info = f"（{sug.get('value', '未知')}）" if sug.get('value') else ""
        suggestion_text += f"{i}. {sug['key']} {value_info}\n"
    return f"⚠️ {error_msg}{suggestion_text}\n💡 請提供完整的地址（包含樓層）以獲得準確結果。"
```

**用戶體驗：**
```
⚠️ 您輸入的地址不夠完整，找到多個可能的匹配。

**找到以下可能的地址，請選擇或提供完整地址：**
1. 新北市三重區重陽路3段158號四樓 （雙月）
2. 新北市三重區重陽路3段158號二樓 （雙月）
3. 新北市三重區重陽路3段158號三樓 （雙月）
4. 新北市三重區重陽路3段158號一樓 （單月）

💡 請提供完整的地址（包含樓層）以獲得準確結果。
```

---

### 3. 表單重試機制

**問題：** 當 API 返回錯誤（如 ambiguous_match）時，表單被標記為完成，用戶無法重新輸入。

**解決方案：**

#### 3.1 API 響應狀態傳遞
**檔案：** `/Users/lenny/jgb/AIChatbot/rag-orchestrator/services/universal_api_handler.py`

```python
# 檢查 API 響應內容中的 success 字段
if isinstance(result_data, dict) and 'success' in result_data:
    # 如果 API 本身返回失敗，直接傳遞該狀態
    if not result_data.get('success'):
        logger.warning(f"⚠️ API 返回業務失敗: {config.get('endpoint_id')}")
        return result_data  # 直接返回原始結果（包含 error, suggestions 等）
```

#### 3.2 表單狀態管理
**檔案：** `/Users/lenny/jgb/AIChatbot/rag-orchestrator/services/form_manager.py`

```python
# 檢查 API 是否返回需要用戶重新輸入的錯誤
if api_result and not api_result.get('success'):
    error_type = api_result.get('error')

    # 特定錯誤類型：需要用戶重新輸入（不完成表單）
    if error_type in ['ambiguous_match', 'no_match', 'invalid_input']:
        print(f"⚠️ API 返回 {error_type}，保持表單狀態為 COLLECTING")

        # 回退到當前欄位，讓用戶重新輸入
        await self.update_session_state(
            session_id=session_state['session_id'],
            state=FormState.COLLECTING
        )

        return {
            "answer": f"{error_message}\n\n---\n\n{current_field['prompt']}\n\n（或輸入「**取消**」結束填寫）",
            "form_completed": False,
            "needs_retry": True,
            "retry_field": current_field['field_name']
        }
```

**效果：**
- 用戶輸入不完整地址 → 顯示所有選項
- 保持表單狀態 → 用戶可以重新輸入
- 輸入完整地址 → 正常完成查詢

---

## 技術細節

### 修改檔案清單

1. **Lookup API 路由**
   - `/Users/lenny/jgb/AIChatbot/rag-orchestrator/routers/lookup.py`
   - 新增多選項檢測邏輯

2. **Universal API Handler**
   - `/Users/lenny/jgb/AIChatbot/rag-orchestrator/services/universal_api_handler.py`
   - 處理 API 響應狀態傳遞
   - 格式化 ambiguous_match 錯誤訊息

3. **Form Manager**
   - `/Users/lenny/jgb/AIChatbot/rag-orchestrator/services/form_manager.py`
   - 實現表單重試機制
   - 管理表單狀態流轉

### 配置參數

| 參數 | 值 | 說明 |
|------|-----|------|
| `ambiguous_threshold` | 0.02 | 相似度差距閾值（2%），低於此值視為模糊不清 |
| `fuzzy_threshold` | 0.75 | 模糊匹配最低相似度 |
| `max_suggestions` | 5 | 最多顯示建議數量 |

### 錯誤類型

| 錯誤類型 | 觸發條件 | 行為 |
|----------|----------|------|
| `ambiguous_match` | 多個相似度接近的匹配 | 保持表單狀態，顯示所有選項 |
| `no_match` | 無法找到匹配 | 保持表單狀態，顯示建議 |
| `invalid_input` | 輸入格式無效 | 保持表單狀態，要求重新輸入 |

---

## 測試驗證

### 精確匹配測試
```bash
# 輸入：新北市三重區重陽路3段158號一樓
# 預期：精確匹配，返回「單月」
# 結果：✅ 通過
```

### 模糊匹配測試（不完整地址）
```bash
# 輸入：新北市三重區重陽路3段158號樓
# 預期：返回 4 個選項（一、二、三、四樓）
# 結果：✅ 通過
```

### 表單重試測試
```bash
# Step 1: 輸入不完整地址 → 顯示選項
# Step 2: 重新輸入完整地址 → 成功查詢
# 結果：✅ 通過（API 層面）
```

---

## 部署說明

### 重啟服務
```bash
docker-compose -f docker-compose.prod.yml restart rag-orchestrator
```

### 資料庫清理（一次性執行）
```bash
# 已在 2026-02-04 執行完成，無需重複執行
# 清理了 19 筆記錄，247 筆資料保持完整
```

---

## 影響範圍

- ✅ **向後兼容**：完整地址查詢不受影響
- ✅ **用戶體驗提升**：不完整地址時給予明確提示
- ✅ **資料完整性**：所有記錄保留，僅清理格式
- ✅ **錯誤率降低**：避免錯誤猜測樓層

---

## 未來優化建議

1. **地址標準化**：建立標準地址格式規範
2. **智能提示**：根據輸入自動補全樓層選項
3. **使用率分析**：追蹤模糊匹配觸發頻率
4. **批量清理工具**：建立自動化地址清理腳本

---

## 維護者

- 開發：Claude Code
- 日期：2026-02-04
- 相關議題：地址匹配優化、表單重試機制
