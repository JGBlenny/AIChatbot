-- Migration 35: 修復審核流程 - 移除 title 欄位
-- 日期: 2025-10-27
-- 問題: approve_ai_knowledge_candidate 函數引用了不存在的 title 欄位
-- 解決: 從 INSERT 語句中移除 title 欄位
...
COMMENT ON FUNCTION approve_ai_knowledge_candidate IS '批准 AI 生成的知識候選，轉為正式知識庫內容（使用候選的 intent_ids，不含 title 欄位）';
...
VALUES ('35-fix-approval-remove-title-field.sql', 'Fix approval function to remove non-existent title field', NOW())
...
-- 1. 在審核中心測試批准一個候選知識
-- 2. 驗證知識庫記錄：
--    SELECT id, question_summary, category, intent_id, source_type
--    FROM knowledge_base
